...
release=struct('started',started,'finished',NaT,'pass',false,'errors',{{}}, ...
    ...
    'robustnessPass',false,'legacyPass',false,'safetyPass',false, ...
    'deterministic',[],'exactReplay',[],'robustness',[],'legacy',[]);

fprintf('\n============================================================\n');
fprintf(' S2.3 v1.0.0 COMPLETE RELEASE GATE\n');
fprintf(' Legacy mode: %s\n',ternary(runLegacy,'full rerun','verify saved evidence'));
fprintf('============================================================\n');
...
% Avoid spending another full legacy campaign when an S2.3 gate is already
% red. The final green release run still executes the complete legacy suite.
preLegacyPass=release.pythonBacktestPass&&release.deterministicPass&& ...
    release.exactReplayPass&&release.robustnessPass;
if preLegacyPass
    try
        if runLegacy
            ...
            release.legacy=verify_S2_2_existing_evidence_S2_3();
            release.legacyPass=logical(release.legacy.pass);
        end
    catch ME
        release.errors{end+1}=getReport(ME,'extended','hyperlinks','off');
...
elseif runLegacy
    release.legacyPass=false;
    release.errors{end+1}='Legacy rerun skipped because one or more S2.3 gates failed.';
end
