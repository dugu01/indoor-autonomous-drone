...
% MISSION_LIFECYCLE_MANAGER_S2_2 Stage S2.2 v0.5.2 autonomous lifecycle.
%
% Flow:
...
for k=2:maxSteps
    ...
    % As soon as all horizontal-aid lanes become ineligible, pause the
    % mission and damp residual horizontal velocity without using a stale XY
    % position target. If aiding recovers before the configured delay, resume
    % the paused state. If it does not, the ESKF raises rtlRequested and the
    % local emergency landing begins below.
    if est.degraded&&armed&&~est.rtlRequested&& ...
            ~any(strcmp(state,{'NAV_DEGRADED_HOLD','EMERGENCY_HOLD', ...
    ...
    if est.rtlRequested&&armed&&~any(strcmp(state,{ ...
        ...
        if ~isfinite(estimatorPositionErrorAtFailsafeTrigger)
            captureFailsafeTriggerMetrics=true;
        end
        [state,stateEntryTime,emergencyLanding,failsafe,holdPosition,verticalStartZ,verticalStartTime]= ...
            begin_emergency_landing(t,est,'EMERGENCY_HOLD');
        pendingReplan=false;
    ...
    switch state
        case 'NAV_DEGRADED_HOLD'
            ...
            ref3=struct('p',[est.p(1:2);holdPosition(3)], ...
                ...
                'horizontalVelocityDampingEnabled',cfg.emergencyVelocityDampingEnabled);
            if ~est.degraded
                state=navigationResumeState;stateEntryTime=t;arrivalConfirmTimer=0;
                navigationResumeState='';
            end
