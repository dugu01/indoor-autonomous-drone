function varargout = multi_lane_eskf_lifecycle_S2_2(action,varargin)
% MULTI_LANE_ESKF_LIFECYCLE_S2_2 Lifecycle-only four-lane navigation bridge.
% This variant starts aid freshness invalid and exposes accepted-aid ages,
% covariance, eligibility and update counts for autonomous preflight gates.
% The validated v0.4 regressions continue to use multi_lane_eskf_S2_2.m unchanged.
% Lanes:
...
if packet.hasVio
    ...
    if isfield(cfg,'initialPosition')&&numel(cfg.initialPosition)==3
        p0=cfg.initialPosition(:);
    else
        p0=[3;0.8;cfg.altitudeNominal_m];
    end
    v0=zeros(3,1);q0=[1 0 0 0];
end
...
prototype=struct('id',0,'name','','imuId',0,'useVio',false,'useLidar',false, ...
    ...
    'nisType',{{}},'acceptedCount',0,'rejectedCount',0,'lastHorizontalAidTime',-inf, ...
    'lastVerticalAidTime',-inf,'lastAttitudeAidTime',-inf,'score',inf,'eligible',false,'reason','initialising');
...
for i=1:4
    ...
    % Freshness timestamps remain -Inf at construction. The initial VIO
    % sample may seed the nominal state, but it is not counted as an accepted
    % filter update. Only measurements that pass filter_update below can make
    % an aid fresh for preflight or lane eligibility.
end
selector=struct('activeLane',1,'candidateLane',0,'candidateSince',-inf,'lastSwitchTime',-inf, ...
    ...
    'blendStartTime',-inf,'maxSwitchJump',0);
nav=struct('lanes',lanes,'selector',selector,'imuDisagreementCount',0,'imuSuspect',-1, ...
    ...
    'switchLog',struct('time',{},'from',{},'to',{},'oldScore',{},'newScore',{},'reason',{}));
end
...
if nav.imuDisagreementCount>=cfg.imuDisagreementSamples
    % Attribute the disagreement using the two all-aid lanes. Lane 1 and
    % Lane 2 receive the same VIO/LiDAR/range/baro information but use
    % different IMUs, so their innovation/covariance scores provide a
    % causal comparison without using truth or a scripted fault label.
    [scorePrimary,~,~]=lane_health(nav.lanes(1),t,cfg);
    [scoreBackup,~,~]=lane_health(nav.lanes(2),t,cfg);
...
old=nav.output;
[nav.selector,scores,eligible,degraded,nav.lanes,sw]=choose_lane(nav.lanes,nav.selector,t,cfg,old,nav.imuSuspect);
...
function [selector,scores,eligible,degraded,lanes,sw]=choose_lane(lanes,selector,t,cfg,old,imuSuspect)
...
selector.offsetP=old.p-lanes(best).p;selector.offsetV=old.v-lanes(best).v;selector.offsetQ=qmul_S2_2(qconj_S2_2(lanes(best).q),old.q);selector.blendStartTime=t;selector.maxSwitchJump=max(selector.maxSwitchJump,norm(selector.offsetP));
sw=struct('time',t,'from',oldId,'to',best,'oldScore',scores(oldId),'newScore',scores(best),'reason',lanes(oldId).reason);
end
...
if isfinite(nav.selector.blendStartTime)
    ...
    if elapsed<cfg.outputBlendTime
        rem=1-elapsed/cfg.outputBlendTime;p=p+rem*nav.selector.offsetP;v=v+rem*nav.selector.offsetV;q=qmul_S2_2(q,qexp_S2_2(rem*qlog_S2_2(nav.selector.offsetQ)));
    end
end
...
horizontalAidAge_s=max(0,t-L.lastHorizontalAidTime);
verticalAidAge_s=max(0,t-L.lastVerticalAidTime);
attitudeAidAge_s=max(0,t-L.lastAttitudeAidTime);
xyCovariance=trace(Pxy);
zVariance=max(L.P(3,3),0);
out=struct('p',p,'v',v,'q',q,'omega',omega,'activeLane',nav.selector.activeLane, ...
    ...
    'horizontalAidAge_s',horizontalAidAge_s,'verticalAidAge_s',verticalAidAge_s, ...
    'attitudeAidAge_s',attitudeAidAge_s,'activeLaneEligible',logical(L.eligible), ...
    'xyCovariance',xyCovariance,'zVariance',zVariance, ...
    'acceptedMeasurementCount',L.acceptedCount,'rejectedMeasurementCount',L.rejectedCount);
end
