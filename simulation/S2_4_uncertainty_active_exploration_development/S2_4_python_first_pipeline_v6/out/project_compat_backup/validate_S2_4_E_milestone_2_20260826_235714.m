...
selectedTargetRelevant=false;distinctDecoyPresent=false;cleanDecoyPresent=false;
tierPriorityRecorded=false;physicalDecoyMoreInformative=false;cleanDecoy=struct();
if hasCompetition
    ...
    [cleanDecoyPresent,cleanDecoy]=cleanTier3Decoy(e);
...
for i=1:numel(history)
    ...
    q=item.evidence;
    if q.hasCompetition&&q.selected&&q.selectedTargetRelevant&&q.distinctIrrelevantFrontierCount>=1
        e=q;return
    end
end
end
