...
if hasCoupledCompetition
    ...
    mostInformativeDecoyPresent=e.distinctIrrelevantFrontierCount>=1&& ...
        isfield(e,'mostInformativeIrrelevantFrontierTrackId')&& ...
        e.mostInformativeIrrelevantFrontierTrackId~=0&& ...
        e.mostInformativeIrrelevantFrontierTrackId~=e.selectedFrontierTrackId;
    decoyTempting=mostInformativeDecoyPresent&& ...
        e.mostInformativeIrrelevantInformationGain>e.selectedInformationGain;
...
if hasCoupledCompetition
    ...
    fprintf('Selected information gain        : %.6f\n', ...
        e.selectedInformationGain);
    fprintf('Best-utility decoy IG / utility  : %.6f / %.6f\n', ...
        e.bestIrrelevantInformationGain,e.bestIrrelevantUtility);
    fprintf('Most-informative decoy IG        : %.6f\n', ...
        e.mostInformativeIrrelevantInformationGain);
    fprintf('Most-informative decoy frontier  : %d\n', ...
        e.mostInformativeIrrelevantFrontierTrackId);
    fprintf('Raw utility margin vs best utility decoy: %.6f\n', ...
...
% The inherited S2.3 truth world may legitimately contain static geometry in
% addition to the two Scenario-2 walls.  The release contract is therefore
% set inclusion: every expected literal wall must be present in the final
% truth world.  Requiring the literal walls to be the ONLY static rectangles
% incorrectly rejects a valid inherited world.
tf=isstruct(world)&&isfield(world,'staticRects5')&& ...
    size(world.staticRects5,2)>=4;
if ~tf,return,end
actual=double(world.staticRects5(:,1:4));
expected=double(expected);
tol=1e-10;
for i=1:size(expected,1)
    delta=max(abs(actual-expected(i,:)),[],2);
    if ~any(delta<tol)
        tf=false;
        return
    end
