...
%
% Video output is verified after the writer is closed. If MPEG-4 is not
% available, the function automatically falls back to Motion JPEG AVI.
%
% Example
...
videoFile=fullfile(results_dir,sprintf('S2_visual_slam_%s_flight_animation.mp4',labelLower));
...
if isempty(fig)||~isgraphics(fig)
    fig=figure('Name','Stage S2 — 6-DOF flight animation', ...
...
title(tl,sprintf('Stage S2 — 6-DOF drone motion (%s)',label), ...
    'FontWeight','normal');
...
if opt.RecordVideo
    try
        ...
        writerOpen=true;
        recording=true;
        fprintf('[VIDEO] Recording to: %s\n',videoFile);
    catch MEmp4
        % Some MATLAB/platform combinations do not provide MPEG-4. Fall
        % back to Motion JPEG AVI instead of silently losing the video.
        warning('Could not open MPEG-4 writer: %s. Trying Motion JPEG AVI.',MEmp4.message);
        videoFile=fullfile(results_dir, ...
            sprintf('S2_visual_slam_%s_flight_animation.avi',labelLower));
        try
            ...
            writerOpen=true;
            recording=true;
            fprintf('[VIDEO] Recording fallback AVI to: %s\n',videoFile);
        catch MEavi
            warning('Could not open any video writer: %s. Animation will still display.',MEavi.message);
            writer=[];
        end
    end
end
writerCleanup=onCleanup(@closeVideoWriter); %#ok<NASGU>
...
for frameNo=1:numel(frames)
    ...
    drawnow limitrate;
    if recording&&writerOpen&&isgraphics(fig)
        try
            frame=getframe(fig);
            frame=makeEvenVideoFrame(frame);
            writeVideo(writer,frame);
            framesWritten=framesWritten+1;
        catch ME
            warning('Video frame write stopped after %d frames: %s',framesWritten,ME.message);
            recording=false;
            closeVideoWriter();
        end
    end
...
videoSaved=false;
if opt.RecordVideo && framesWritten>0 && exist(videoFile,'file')==2
    videoInfo=dir(videoFile);
    videoSaved=videoInfo.bytes>0;
    if videoSaved
        fprintf('[VIDEO] Saved %d frames (%0.2f MB): %s\n', ...
            framesWritten,videoInfo.bytes/1024^2,videoFile);
    else
        warning('Video file was created but is empty: %s',videoFile);
    end
elseif opt.RecordVideo
    warning('Video was not saved. Frames written: %d. Expected file: %s', ...
        framesWritten,videoFile);
end

if isgraphics(fig)
    if videoSaved
        finalStatus=sprintf('%s animation complete. Video saved: %s',label,videoFile);
    elseif opt.RecordVideo
        finalStatus=sprintf('%s animation complete, but video was not saved. See Command Window.',label);
    else
        finalStatus=sprintf('%s animation complete. Video recording disabled.',label);
    end
    set(statusText,'String',finalStatus);
    set(pauseButton,'Enable','off');set(stopButton,'Enable','off');

    % A figure containing both a uipanel/tiledlayout and uicontrols cannot
    % be passed to exportgraphics as one container. Capture the rendered
    % window with getframe for PNG, and save the FIG independently.
    pngFile=fullfile(results_dir, ...
        sprintf('S2_visual_slam_%s_animation_final.png',labelLower));
    try
        finalFrame=getframe(fig);
        imwrite(finalFrame.cdata,pngFile);
    catch MEpng
        warning('Could not save final animation PNG: %s',MEpng.message);
        try
            exportgraphics(ax3,pngFile,'Resolution',180);
        catch MEaxes
            warning('Could not save 3-D animation axes either: %s',MEaxes.message);
        end
    end

    figFile=fullfile(results_dir, ...
        sprintf('S2_visual_slam_%s_animation_window.fig',labelLower));
    try
        savefig(fig,figFile);
    catch MEfig
        warning('Could not save final animation FIG: %s',MEfig.message);
    end
    ...
    function closeVideoWriter()
        % Close exactly once. Closing the writer finalizes the MP4/AVI
        % container and makes the file visible to the operating system.
        if writerOpen && ~isempty(writer)
            try
                close(writer);
            catch MEclose
                warning('Could not finalize video writer: %s',MEclose.message);
            end
            writerOpen=false;
            recording=false;
        end
...
function frame=makeEvenVideoFrame(frame)
% H.264 requires even image dimensions. Crop at most one pixel from the
% right/bottom so MATLAB does not issue codec-padding warnings.
if isempty(frame) || ~isfield(frame,'cdata') || isempty(frame.cdata),return;end
[h,w,~]=size(frame.cdata);
frame.cdata=frame.cdata(1:h-mod(h,2),1:w-mod(w,2),:);
if isfield(frame,'colormap'),frame.colormap=[];end
end
