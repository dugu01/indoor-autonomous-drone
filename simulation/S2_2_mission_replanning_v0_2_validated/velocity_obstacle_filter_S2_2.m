...
% dynStates is a struct array with p, v and radius fields.
...
bestScore=inf;bestEscape=-inf;cmd=[0 0];minMargin=-inf;minTTC=inf;found=false;
for i=1:size(candidates,1)
    ...
    if escapeScore>bestEscape
        bestEscape=escapeScore;escapeCand=cand;escapeMargin=margin;escapeTTC=ttc; %#ok<NASGU>
    end
    if staticOK && margin>=0
        score=norm(cand-desired)^2-0.08*min(margin,2.0)-0.03*norm(cand);
        if score<bestScore
            bestScore=score;cmd=cand;minMargin=margin;minTTC=ttc;found=true;
        end
    end
end
if ~found
    cmd=escapeCand;minMargin=escapeMargin;minTTC=escapeTTC;
    if minMargin<0 && norm(cmd)<1e-9,cmd=[0 0];end
end
