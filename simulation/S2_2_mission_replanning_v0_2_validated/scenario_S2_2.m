...
% SCENARIO_S2_2  Stage S2.2 v0.2 validation scenarios.
...
scenario.dynamicObstacles = struct('start',{},'velocity',{},'radius',{}, ...
    'appearTime',{},'disappearTime',{},'stopTime',{});
scenario.sensorDropoutWindows = zeros(0,2);
scenario.expectedGoalReached = true;
scenario.expectedFailsafe = false;
scenario.expectedIncrementalReplan = false;
scenario.expectedDynamicAvoidance = false;
scenario.expectedNoDataHold = false;
...
switch key
    case {'incremental_static_insert','incremental'}
        ...
        scenario.expectedIncrementalReplan = true;
    ...
    case {'sensor_dropout_recover','dropout_recover'}
        scenario.name = 'SENSOR_DROPOUT_RECOVER';
        scenario.sensorDropoutWindows = [5.0 6.2];
        scenario.expectedNoDataHold = true;

    case {'sensor_dropout_failsafe','dropout_failsafe'}
        scenario.name = 'SENSOR_DROPOUT_FAILSAFE';
        scenario.sensorDropoutWindows = [5.0 12.0];
    ...
    otherwise
        error('S2_2:UnknownScenario','Unknown S2.2 v0.2 scenario: %s',name);
end
end
