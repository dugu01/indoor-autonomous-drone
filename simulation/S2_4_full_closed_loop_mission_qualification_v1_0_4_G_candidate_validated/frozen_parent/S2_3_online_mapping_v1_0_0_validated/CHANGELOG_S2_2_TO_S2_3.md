...
## Exact-replay-gated known-boundary integration

The instrumented MATLAB nominal run captured 1342 accepted raw perception
packets. Exact mapper replay reproduced every mapper array and all production
map metrics. The only old replay mismatch was the informational idle-control
counter, which is not present in an accepted-packet stream.

A counterfactual MATLAB replay then registered only the already-known
room/geofence boundary from `cfg.room` before inserting the same 1342 packets.
That replay passed with zero packet rejections and improved the production
metrics from 0.005818 false-free / 0.967483 recall to 0.00174985 / 0.990054.
The coupled candidate therefore integrates exactly that verified boundary
contract in `init_probabilistic_map_S2_3.m`.

No unknown obstacle truth, flight threshold, clearance margin, estimator,
controller, planner core, or lifecycle threshold was changed.

## Release end-to-end closure
- Replaced unsafe late obstacle teleportation with a sensor-only late corridor blockage at 12 s with verified reaction margin and alternate route.
- Made map extension optional in the visible narrow-passage scenario.
- Added complete 12-scenario source, geometry, visibility, timing, dropout, estimator and dynamic-promotion backtest.
