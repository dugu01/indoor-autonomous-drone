...
Run MATLAB from the extracted S2.4 release-candidate root. Resolve the root from the current folder rather than a machine-specific absolute path:

```matlab
projectRoot = pwd;
assert(isfile(fullfile(projectRoot,'setup_S2_4_E_path.m')), ...
    'Start MATLAB from the S2.4 release-candidate root.');
```
