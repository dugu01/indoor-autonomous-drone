...
% Resolve the S2.4 package root from this script itself.  Do not hard-code
% a development-folder path: the release candidate must be runnable after
% extraction/renaming without falling back to another workspace.
s24root = fileparts(mfilename('fullpath'));
assert(~isempty(s24root) && isfolder(s24root), ...
    'Could not resolve the S2.4 package root from run_validate_S2_4_AD_all.m.');
...
% The frozen S2.3 parent contains two manifest-tracked CPython bytecode
% files.  Validation must never regenerate/add __pycache__ content inside
% the frozen parent, regardless of the Python version MATLAB resolves.
% Propagate this to every Python subprocess launched during this session.
...
try
    ...
    [status, text] = system('python3 -B --version');
    fprintf('%s\n', text);
    ...
    command = sprintf( ...
        'cd "%s" && PYTHONDONTWRITEBYTECODE=1 python3 -B tools/run_all_checks.py', ...
        s24root);
    ...
    command = sprintf( ...
        ['cd "%s" && ' ...
         'PYTHONDONTWRITEBYTECODE=1 python3 -B tools/audit_parent_immutability.py && ' ...
         'PYTHONDONTWRITEBYTECODE=1 python3 -B tools/audit_final_parent_manifest.py'], ...
        s24root);
