...
ck('candidate_version_v1_0_8','v1.0.8-estimation-perception-robustness-candidate' in cfg)
# Config is byte-equivalent to v1.0.6 except version string.
ref=(E/'mission__init_S2_5_config.m.txt').read_text().replace('v1.0.6-estimation-perception-robustness-candidate','v1.0.8-estimation-perception-robustness-candidate')
...
ck('state_context_passed_to_actual_packet_generation',mgr.count("'lifecycleState',state")>=2 and "'lifecycleState','PREFLIGHT'" in mgr, f'count={mgr.count(chr(39)+"lifecycleState"+chr(39)+",state")}')
...
ok=all(c for _,c,_ in checks); print('S2.5 v1.0.8 STATIC / ISOLATION AUDIT:', 'PASS' if ok else 'FAIL'); sys.exit(0 if ok else 1)
