function report = run_S2_5_v1_0_5_historical_preflight()
% RUN_S2_5_V1_0_5_HISTORICAL_PREFLIGHT Run only the five exact historical
...
fprintf('\n============================================================\n');
fprintf(' S2.5 v1.0.5 — STATIC + EXACT-SNAPSHOT OFFLINE PREFLIGHT\n');
fprintf('============================================================\n');
cmds={ ...
 fullfile(root,'s2_5','validation','audit_S2_4_G_parent_immutability.py'), ...
 fullfile(root,'s2_5','validation','audit_S2_5_static_v1_0_5.py'), ...
 fullfile(root,'s2_5','validation','audit_S2_5_matlab_sources.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_fault_model.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_recovery_logic_v1_0_5.py'), ...
 fullfile(root,'s2_5','validation','exact_snapshot_replay_v1_0_5','replay_exact_snapshots.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_parallel_harness.py')};
for k=1:numel(cmds)
    [rc,out]=system(sprintf('python3 "%s"',cmds{k}));fprintf('%s',out);
    assert(rc==0,'S2_5:V105OfflineGate','Offline gate failed: %s',cmds{k});
end
poolInfo=start_S2_5_parallel_pool();

fprintf('\n============================================================\n');
fprintf(' S2.5 v1.0.5 — PARALLEL HISTORICAL RUNTIME PREFLIGHT (5)\n');
fprintf('============================================================\n');
...
assert(report.pass,'S2_5:HistoricalRecoveryFailure','One or more v1.0.5 historical cases failed. Do NOT launch the full matrix.');
fprintf('\n============================================================\n');
fprintf(' S2.5 v1.0.5 HISTORICAL PREFLIGHT: PASS 5/5\n');
