...
ck('actual_packet_generation_receives_lifecycle_state',mgr.count("'lifecycleState',state")>=2,f'count={mgr.count(chr(39)+"lifecycleState"+chr(39)+",state")}')
for n,c,d in checks: print(f'{n:50s} {"PASS" if c else "FAIL"} {d}')
ok=all(c for _,c,_ in checks); print('S2.5 v1.0.8 LIFECYCLE / FAULT-PHASING BACKTEST:', 'PASS' if ok else 'FAIL'); sys.exit(0 if ok else 1)
