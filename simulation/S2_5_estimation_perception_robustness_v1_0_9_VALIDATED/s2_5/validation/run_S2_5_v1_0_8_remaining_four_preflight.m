function report = run_S2_5_v1_0_8_remaining_four_preflight()
...
cmds={fullfile(root,'s2_5','validation','audit_S2_4_G_parent_immutability.py'), ...
 fullfile(root,'s2_5','validation','audit_S2_5_static_v1_0_8.py'), ...
 fullfile(root,'s2_5','validation','audit_S2_5_matlab_sources.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_v1_0_8_lifecycle_fault_phasing.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_recovery_logic_v1_0_8.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_v1_0_6_perception_integrity.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_parallel_harness_v1_0_8.py')};
fprintf('\n============================================================\n S2.5 v1.0.8 — OFFLINE + REMAINING FOUR PREFLIGHT\n============================================================\n');
...
pOld=gcp('nocreate');if ~isempty(pOld),fprintf('S2.5 v1.0.8: restarting pool to prevent stale-package workers.\n');delete(pOld);end
...
assert(report.pass,'S2_5:V107RemainingFourFailure','One or more v1.0.8 remaining-four cases failed. Do NOT run full matrix.');
fprintf('\nS2.5 v1.0.8 REMAINING FOUR: PASS 4/4\nSafe to proceed to: gate = validate_S2_5_all();\n');
end
