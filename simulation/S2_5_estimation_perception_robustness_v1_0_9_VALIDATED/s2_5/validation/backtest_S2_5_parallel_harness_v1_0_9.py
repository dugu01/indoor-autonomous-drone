...
# A. v1.0.9 intentionally changes the S2.5 manager only for the reviewed
# pre-map integrity overlay and separate S2.5 recovery scan accounting.
manager_text=manager.read_text()
ck('v109_manager_integrity_overlay_present','sanitize_perception_packet_S2_5' in manager_text)
ck('v109_manager_separate_scan_accounting','s25RecoveryScanHoldPass' in manager_text and 's25MappingCompositePass' in manager_text)
...
# C. v1.0.9 acceptance semantics preserve all inherited gates and extend only
# the S2.5 brief-perception evidence from a named MAP_DEGRADED_HOLD to either
# that hold or a stale-perception episode while already in the zero-motion
# LIFECYCLE_REPLAN_BRAKE safety state. Prolonged failsafe still requires the
# actual perception hold.
ck('runner_safe_perception_response_gate',
   "field_or_local(s,'s25PerceptionSafeResponseCount',s.perceptionHoldCount)>=1" in runner)
ck('validator_safe_perception_helper','function n=safe_perception_response_count(s)' in val)
ck('historical_brief_uses_safe_response','extra=safe_perception_response_count(s)>=1;' in val)
ck('stale_burst_requires_reject_and_safe_response','extra=s.mapRejectedPackets>=1&&safe_perception_response_count(s)>=1;' in val)
ck('failsafe_prolonged_still_requires_actual_hold','evidence=evidence&&s.perceptionHoldCount>=1&&s.s25PerceptionFaultApplicationCount>=1;' in val)
for tok in ['s.goalReached&&~s.failsafeTriggered&&extra','s.executionSafetyPass','~s.stateTimeoutTriggered']:
    ck('acceptance_retains_'+re.sub(r'\W+','_',tok),tok in val or tok in runner)
...
def main():
    ...
    print(f'\nS2.5 v1.0.9 PARALLEL HARNESS PYTHON BACKTEST: {"PASS" if passed else "FAIL"}')
