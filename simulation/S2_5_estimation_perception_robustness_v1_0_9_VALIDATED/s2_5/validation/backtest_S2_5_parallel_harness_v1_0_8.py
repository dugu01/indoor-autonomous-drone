...
# A. v1.0.8 intentionally changes the S2.5 manager only for the reviewed
# pre-map integrity overlay and separate S2.5 recovery scan accounting.
manager_text=manager.read_text()
ck('v108_manager_integrity_overlay_present','sanitize_perception_packet_S2_5' in manager_text)
ck('v108_manager_separate_scan_accounting','s25RecoveryScanHoldPass' in manager_text and 's25MappingCompositePass' in manager_text)
...
def main():
    ...
    print(f'\nS2.5 v1.0.8 PARALLEL HARNESS PYTHON BACKTEST: {"PASS" if passed else "FAIL"}')
