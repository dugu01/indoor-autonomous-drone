...
req=[S/'mission/init_S2_5_config.m',S/'mission/run_S2_5_coupled.m',S/'mission/mission_lifecycle_manager_S2_5.m',S/'mission/plan_recovery_viewpoint_S2_5.m',
 ...
 S/'validation/backtest_S2_5_recovery_logic_v1_0_5.py',S/'validation/backtest_S2_5_parallel_harness.py',
...
ck('v105_root_preflight_wrapper',(ROOT/'run_validate_S2_5_v1_0_5_preflight.m').exists())
...
ck('candidate_version_v1_0_5',"v1.0.5-estimation-perception-robustness-candidate" in config)
...
old_cfg=config.replace('v1.0.5-estimation-perception-robustness-candidate','v1.0.1-estimation-perception-robustness-candidate')
...
# v1.0.5 is intentionally a minimal overlay on v1.0.4-r1: lifecycle/fault/harness runtime files remain exact.
v104_runtime_hashes={
 'mission/mission_lifecycle_manager_S2_5.m':'507e6e1ede1880ef571f03562555f717f52ea6ec90606f3ddf38186a12c6b401',
 'mission/run_S2_5_coupled.m':'7da34d2b57e5cb42374cd6471226d2b70f1af6c7774a549b642f57c5ce091edb',
 'scenarios/scenario_S2_5.m':'39837a9a1fa4a57d141172171dac0c59f801b7c1dd7c48f1ece232584d3337e0',
 'sensors/simulate_sensor_packet_S2_5.m':'2fc21bea1b228821dc106325f5b46495c8b84dcb496caef6752d4aba1d709d02',
 'perception/simulate_perception_packet_S2_5.m':'a7cad1065a52a08a7a3de84e1157a472f468973b49bb412c54a53db560908b36',
...
for rel,h in v104_runtime_hashes.items(): ck('v104_runtime_exact_'+rel.replace('/','_').replace('.','_'),sha(S/rel)==h)
validator=(S/'validation/validate_S2_5_all.m').read_text()
ck('v105_validator_uses_v105_static','audit_S2_5_static_v1_0_5.py' in validator)
ck('v105_validator_uses_v105_recovery','backtest_S2_5_recovery_logic_v1_0_5.py' in validator)
ck('v105_validator_requires_exact_snapshot_gate','exact_snapshot_replay_v1_0_5' in validator and 'rcX==0' in validator)
ck('v105_five_only_runtime_preflight',(S/'validation/run_S2_5_v1_0_5_historical_preflight.m').exists())
...
ck('v105_cse_continuous_only',all(x in helper for x in ['continuous_start_egress_local','continuous_segment_safe_local','startRoundedBlocked']))
ck('v105_authoritative_grid_not_cleared','grid.occ(start(1),start(2))=false;' not in helper and 'planGrid.occ(start(1),start(2))=false;' in helper)
ck('v105_unknown_remains_continuous_hazard','logical(g.staticOccupiedRaw)|logical(g.dynamicOccupiedRaw)|logical(g.unknown)' in helper)
ck('v105_info_scored_along_route','best_information_anchor_local' in helper and 'prepare_path_preserve_anchor_local' in helper)
ck('v105_separate_stop_terminal_fallback','terminal_order_local' in helper and 'selectedStage=2' in helper)
ck('v105_exact_snapshot_replay_present',(S/'validation/exact_snapshot_replay_v1_0_5/replay_exact_snapshots.py').exists())
...
print('S2.5 v1.0.5 STATIC / ISOLATION AUDIT:', 'PASS' if passed else 'FAIL')
