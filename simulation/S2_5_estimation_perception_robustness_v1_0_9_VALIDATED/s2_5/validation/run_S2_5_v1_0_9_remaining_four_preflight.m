function report = run_S2_5_v1_0_9_remaining_four_preflight()
...
cmds={fullfile(root,'s2_5','validation','audit_S2_4_G_parent_immutability.py'), ...
 fullfile(root,'s2_5','validation','audit_S2_5_static_v1_0_9.py'), ...
 fullfile(root,'s2_5','validation','audit_S2_5_matlab_sources.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_v1_0_9_lifecycle_integration.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_recovery_logic_v1_0_9.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_v1_0_6_perception_integrity.py'), ...
 fullfile(root,'s2_5','validation','backtest_S2_5_parallel_harness_v1_0_9.py')};
fprintf('\n============================================================\n S2.5 v1.0.9 — OFFLINE + REMAINING FOUR PREFLIGHT\n============================================================\n');
for k=1:numel(cmds),[rc,o]=system(sprintf('python3 "%s"',cmds{k}));fprintf('%s',o);assert(rc==0,'S2_5:V109OfflineGate','Offline gate failed: %s',cmds{k});end
...
for k=1:n
 ...
 fprintf('      perceptionSafeResponse=%d staleBrakeEpisodes=%d navDegradedHolds=%d\n',field_local(s,'s25PerceptionSafeResponseCount',s.perceptionHoldCount),field_local(s,'s25PerceptionStaleBrakeEpisodeCount',0),field_local(s,'navigationDegradedHoldCount',0));
end
...
assert(report.pass,'S2_5:V109RemainingFourFailure','One or more v1.0.9 remaining-four cases failed. Do NOT run full matrix.');
fprintf('\nS2.5 v1.0.9 REMAINING FOUR: PASS 4/4\nSafe to proceed to: gate = validate_S2_5_all();\n');
end
function s=pf(x),if x,s='PASS';else,s='FAIL';end,end
function v=field_local(s,n,d),if isstruct(s)&&isfield(s,n),v=s.(n);else,v=d;end,end
