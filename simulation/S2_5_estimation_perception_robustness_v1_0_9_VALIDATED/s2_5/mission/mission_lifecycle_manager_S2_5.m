function [log,summary,maps] = mission_lifecycle_manager_S2_5(cfg,scenario)
% MISSION_LIFECYCLE_MANAGER_S2_5 S2.4-G runtime with S2.5 sensor/perception fault overlay.
%
% This file is a development copy of the validated S2.3 lifecycle manager.
% The frozen parent remains unchanged. The only new execution path is an
% outbound mission-level request to a safe S2.4 sensing viewpoint when no
% direct known-free target route exists.
%
...
[packet,sensorModel]=simulate_sensor_packet_S2_5(cfg,scenario,truth,sensorModel,0,1);
...
perceptionReplay={};
truthContext=struct('rtlObstacleActive',false,'homeBlockActive',false,'lifecycleState','PREFLIGHT');
[perceptionPacket,perceptionModel,truthWorld]=simulate_perception_packet_S2_5( ...
    cfg,scenario,truth,perceptionModel,0,1,truthContext);
mapState=init_probabilistic_map_S2_3(cfg);
perceptionIntegrityDuplicateHitRayCount=0;
perceptionIntegrityOcclusionRejectedHitRayCount=0;
perceptionIntegrityKeptHitRayCount=0;
[mapPacket,integrityStats]=sanitize_perception_packet_S2_5(cfg,mapState,perceptionPacket,est);
perceptionIntegrityDuplicateHitRayCount=perceptionIntegrityDuplicateHitRayCount+integrityStats.duplicateHitRayCount;
perceptionIntegrityOcclusionRejectedHitRayCount=perceptionIntegrityOcclusionRejectedHitRayCount+integrityStats.occlusionRejectedHitRayCount;
perceptionIntegrityKeptHitRayCount=perceptionIntegrityKeptHitRayCount+integrityStats.keptHitRayCount;
if ~isfield(cfg,'activeExploration')
    error('S2_4:ConfigMissing','cfg.activeExploration is required.');
end
s24cfg=cfg.activeExploration;
executionSafetyEnabled=isfield(cfg,'executionSafety')&&cfg.executionSafety.enabled;
if executionSafetyEnabled,executionSafetyCfg=cfg.executionSafety;else,executionSafetyCfg=struct();end
uncertaintyState=init_uncertainty_sidecar_S2_4(mapState);
frontierState=struct();
[mapState,lastMapUpdate]=update_probabilistic_map_S2_3( ...
    cfg,mapState,mapPacket,est,0);
if lastMapUpdate.accepted
    replayPose=struct('p',est.p,'q',est.q,'xySigma_m',est.xySigma_m);
    initialRecord=struct('packet',mapPacket, ...
        'pose',replayPose,'callTime',0,'update',lastMapUpdate);
    perceptionReplay{end+1}=initialRecord; %#ok<AGROW>
    uncertaintyState=update_uncertainty_sidecar_S2_4( ...
        s24cfg,uncertaintyState,mapState,initialRecord,lastMapUpdate);
end
...
goalUnreachableReason='NONE';
explorationRequestCount=0;explorationSelectedCount=0;explorationExecutedCount=0;
explorationRejectedDecisionCount=0;unsafeViewpointExecutionCount=0;
explorationRequestHistory={};explorationDecisionHistory={};
executedFrontierTrackIds=uint64([]);
activeExplorationRequest=struct();explorationActive=false;
activeExplorationAcceptedTime=nan;firstExplorationAcceptedTime=nan;
executionRevalidationCount=0;executionRevalidationPassCount=0;executionLeaseRenewalCount=0;
executionAuthorityInvalidationCount=0;executionRetreatRefreshCount=0;
executionMapChangeRevalidationCount=0;executionExpiryInvalidationCount=0;
executionPerceptionRevocationCount=0;staleCommandContinuationCount=0;executionReferenceGuardCount=0;
executionReferenceGuardOutboundCount=0;executionReferenceGuardRTLCount=0;
executionSafetyHistory={};pendingExplorationReplan=false;pendingExplorationTerminal=false;
freshPlanRequiredAfterRevocation=false;postFaultFreshPlanCount=0;
suspendedExplorationRequest=struct();suspendedRequestAvailable=false;suspendedRequestReason='NONE';
executionSuspendedRequestRecoveryCount=0;executionSuspendedRequestRecoveryRejectCount=0;
faultInjectionCount=0;faultInjectedAuthorityGenerations=uint64([]);
firstFaultInjectionTime=nan;firstFaultDetectionTime=nan;lastExecutionSafetyReason='NONE';
postFaultValidRevalidationCount=0;
lastExecutionStoppingDistance_m=nan;lastExecutionRemainingRouteLength_m=nan;
...
informativeRecoveryRelocationCount=0;
informativeRecoveryAttemptCount=0;
lastInformativeRecoveryDiagnostics=struct();
segmentRecoveryRelocation=false;
...
perceptionStaleBrakeEpisodeCount=0;perceptionStaleBrakeActive=false;
unknownCommitmentCount=0;unknownCommitmentOutboundCount=0;unknownCommitmentRTLCount=0;lastPlannerMapVersion=double(mapState.version);
...
maxTruthYawRate_radps=abs(truth.omega(3));
...
Pref=nan(maxSteps,3);Vref=nan(maxSteps,3);Aref=nan(maxSteps,3);yawRefLog=nan(maxSteps,1);trackingLog=nan(maxSteps,1);estErrorLog=nan(maxSteps,1);
...
explorationActiveLog=false(maxSteps,1);explorationRequestIdLog=zeros(maxSteps,1,'uint64');
explorationFrontierIdLog=zeros(maxSteps,1,'uint64');
...
for k=2:maxSteps
    ...
    % G v1.0.4: exploration viewing yaw is a bounded-rate reference, not
    % an angle step. The inherited S2.3 scan policy already qualified a
    % continuous 35 deg/s yaw sweep after a 55 deg/s/resetting reference
    % violated the 2 deg estimator-attitude gate. Reuse that same bound for
    % ordinary TRACK_OUTBOUND orientation changes. RTL retains the last yaw.
    if strcmp(state,'TRACK_OUTBOUND')
        yawCommand=slew_yaw_reference_local(yawCommand,yawTargetCommand, ...
            cfg.mapScanYawRate_radps,cfg.dt);
    end
    ...
    truthContext=struct('rtlObstacleActive',rtlObstacleInserted, ...
        'homeBlockActive',homeBlockInserted,'lifecycleState',state);
    ...
    [mapPacket,integrityStats]=sanitize_perception_packet_S2_5(cfg,mapState,perceptionPacket,mapPose);
    perceptionIntegrityDuplicateHitRayCount=perceptionIntegrityDuplicateHitRayCount+integrityStats.duplicateHitRayCount;
    perceptionIntegrityOcclusionRejectedHitRayCount=perceptionIntegrityOcclusionRejectedHitRayCount+integrityStats.occlusionRejectedHitRayCount;
    perceptionIntegrityKeptHitRayCount=perceptionIntegrityKeptHitRayCount+integrityStats.keptHitRayCount;
    [mapState,lastMapUpdate]=update_probabilistic_map_S2_3( ...
        cfg,mapState,mapPacket,mapPose,t);
    if lastMapUpdate.accepted
        replayPose=struct('p',mapPose.p,'q',mapPose.q,'xySigma_m',mapPose.xySigma_m);
        sidecarRecord=struct('packet',mapPacket, ...
            'pose',replayPose,'callTime',t,'update',lastMapUpdate);
        perceptionReplay{end+1}=sidecarRecord; %#ok<AGROW>
        uncertaintyState=update_uncertainty_sidecar_S2_4( ...
            s24cfg,uncertaintyState,mapState,sidecarRecord,lastMapUpdate);
    end
    ...
    faultMeta=struct('active',false,'applied',false,'name','NONE','detail','NONE');
    % Validation faults are authority-relative, not wall-clock-relative. A
    % fault can be injected only while the accepted exploration request is
    % actually authorizing TRACK_OUTBOUND motion. This guarantees the fault
    % occurs after acceptance and lets F11 wait for a real traversed prefix.
    if executionSafetyEnabled&&explorationActive&&strcmp(state,'TRACK_OUTBOUND')&& ...
            activeAuthorityGeneration>0&&~isempty(fieldnames(activeExplorationRequest))
        fcfg=executionSafetyCfg.validationFault;
        repeatPerAuthority=logical(field_or_local(fcfg,'repeatPerAuthority',false));
        alreadyThisGeneration=any(faultInjectedAuthorityGenerations==activeAuthorityGeneration);
        alreadyAny=~isempty(faultInjectedAuthorityGenerations);
        progress=route_progress_fraction(activeExplorationRequest,est.p(1:2).');
        delayReady=isfinite(activeExplorationAcceptedTime)&& ...
            t>=activeExplorationAcceptedTime+field_or_local(fcfg,'triggerDelay_s',0.10)-1e-12;
        progressReady=progress>=field_or_local(fcfg,'triggerProgress',0.20)-1e-12;
        allowFault=delayReady&&progressReady&&~alreadyThisGeneration&& ...
            (repeatPerAuthority||~alreadyAny);
        if allowFault
            [grid,mapState,activeExplorationRequest,faultMeta]= ...
                apply_validation_fault_S2_4_F(fcfg,grid,mapState, ...
                activeExplorationRequest,est.p(1:2).',t);
        end
        if faultMeta.applied
            faultInjectionCount=faultInjectionCount+1;
            faultInjectedAuthorityGenerations(end+1)=activeAuthorityGeneration; %#ok<AGROW>
            if ~isfinite(firstFaultInjectionTime),firstFaultInjectionTime=t;end
            executionSafetyHistory{end+1}=struct('time',t,'event','FAULT_INJECTED', ...
                'reason',faultMeta.detail,'requestId',active_request_id(activeExplorationRequest), ...
                'authorityGeneration',activeAuthorityGeneration,'routeProgress',progress); %#ok<AGROW>
        end
    end
    ...
    % S2.5 v1.0.9: if perception becomes stale while execution safety has
    % already placed the vehicle in the zero-translation replan brake, count
    % that as a distinct safe degraded-perception response episode. Do not
    % force a redundant MAP_DEGRADED_HOLD transition and do not authorize
    % translation on stale perception.
    if armed&&~perceptionFresh&&strcmp(state,'LIFECYCLE_REPLAN_BRAKE')
        if ~perceptionStaleBrakeActive
            perceptionStaleBrakeEpisodeCount=perceptionStaleBrakeEpisodeCount+1;
        end
        perceptionStaleBrakeActive=true;
    elseif perceptionFresh
        perceptionStaleBrakeActive=false;
    end
    ...
    % F: any perception-degraded transition revokes an active exploration
    ...
    if executionSafetyEnabled&&explorationActive&&strcmp(state,'MAP_DEGRADED_HOLD')
        ...
        executionSafetyHistory{end+1}=struct('time',t,'event','AUTHORITY_REVOKED', ...
        ...
        revokedAuthorityGeneration=activeAuthorityGeneration;activeAuthorityGeneration=uint64(0);
        suspendedExplorationRequest=activeExplorationRequest;suspendedRequestAvailable=true;
        suspendedRequestReason=lastExecutionSafetyReason;
        explorationActive=false;activeExplorationRequest=struct();activeExplorationAcceptedTime=nan;
        % The revoked viewing direction is no longer an authorized motion
        % objective. Hold the current estimated heading during brake/replan
        % so rate damping removes residual yaw motion instead of continuing
        % toward stale exploration yaw.
        yawCommand=estimated_yaw_S2_3(est);yawTargetCommand=yawCommand;
        freshPlanRequiredAfterRevocation=true;
        % A safety/perception revocation is not an exploration no-progress
        % event. Give the inherited planner/scanner its normal bounded
        % recovery budget; repeated authority invalidations remain bounded
        % independently by executionSafety.maxAuthorityInvalidations.
        scanNoProgressCount=0;
    end

    % F: accepted exploration requests are execution leases, not one-time
    % admission checks. Revalidate the remaining route, viewpoint/hold, stop
    % reserve, retreat authority, map version and expiry before future motion.
    if executionSafetyEnabled&&explorationActive&&strcmp(state,'TRACK_OUTBOUND')&& ...
            ~isempty(fieldnames(activeExplorationRequest))
        [executionStatus,refreshedRequest]= ...
            revalidate_active_exploration_request_S2_4_F(cfg,s24cfg, ...
            build_execution_grid_S2_4(grid),activeExplorationRequest, ...
            est.p(1:2).',est.v(1:2),t);
        executionRevalidationCount=executionRevalidationCount+1;
        lastExecutionStoppingDistance_m=executionStatus.stoppingDistance_m;
        lastExecutionRemainingRouteLength_m=executionStatus.remainingRouteLength_m;
        lastExecutionRouteStopReserveSafe=executionStatus.routeStopReserveSafe;
        lastExecutionTerminalOverrunNeeded_m=executionStatus.terminalOverrunNeeded_m;
        lastExecutionTerminalOverrunReserveSafe=executionStatus.terminalOverrunReserveSafe;
        if executionStatus.mapChangedSinceRequest
            executionMapChangeRevalidationCount=executionMapChangeRevalidationCount+1;
        end
        if executionStatus.retreatRefreshed
            executionRetreatRefreshCount=executionRetreatRefreshCount+1;
            executionSafetyHistory{end+1}=struct('time',t,'event','RETREAT_REFRESHED', ...
                'reason','CURRENT_GRID_ASTAR','requestId',active_request_id(activeExplorationRequest)); %#ok<AGROW>
        end
        if executionStatus.valid
            executionRevalidationPassCount=executionRevalidationPassCount+1;
            if isfield(executionStatus,'leaseRenewed')&&executionStatus.leaseRenewed
                executionLeaseRenewalCount=executionLeaseRenewalCount+1;
            end
            if isfinite(firstFaultInjectionTime)&&t>=firstFaultInjectionTime-1e-12
                postFaultValidRevalidationCount=postFaultValidRevalidationCount+1;
            end
            activeExplorationRequest=refreshedRequest;
            lastExecutionSafetyReason='VALID';
            ...
            lastExecutionSafetyReason=executionStatus.reason;
            if ~isfinite(firstFaultDetectionTime)&&faultMeta.applied,firstFaultDetectionTime=t;end
            executionSafetyHistory{end+1}=struct('time',t,'event','AUTHORITY_INVALIDATED', ...
                'reason',executionStatus.reason,'requestId',active_request_id(activeExplorationRequest), ...
                'authorityGeneration',activeAuthorityGeneration); %#ok<AGROW>
            revokedAuthorityGeneration=activeAuthorityGeneration;activeAuthorityGeneration=uint64(0);
            if any(strcmp(executionStatus.reasons,'REQUEST_EXPIRED'))
                % F8: an expired request is dead authority and is not retained
                % even as a recovery proposal. A later motion needs a newly
                % planned request from the normal exploration pipeline.
                suspendedExplorationRequest=struct();suspendedRequestAvailable=false;
                suspendedRequestReason='REQUEST_EXPIRED';
            else
                % Preserve only proposal data. It carries no authority after
                % revocation and can move again only after a current-grid plan,
                % fresh admission check and execution revalidation all pass.
                suspendedExplorationRequest=activeExplorationRequest;
                suspendedRequestAvailable=true;suspendedRequestReason=executionStatus.reason;
            end
            explorationActive=false;activeExplorationRequest=struct();activeExplorationAcceptedTime=nan;
            % F authority is dead immediately; its viewing yaw is dead too.
            % Freeze heading at the current estimate while the vehicle brakes.
            yawCommand=estimated_yaw_S2_3(est);yawTargetCommand=yawCommand;
            freshPlanRequiredAfterRevocation=true;
            % Safety-mandated revalidation/replan must not consume the
            % exploration no-progress watchdog. F14's explicit authority
            % invalidation bound still prevents unsafe livelock.
            scanNoProgressCount=0;
    ...
    if routeAffected&&any(strcmp(state,{'TRACK_OUTBOUND','TRACK_RTL'}))
        ...
        elseif routeExists
            ...
            replanBrakeCount=replanBrakeCount+1;state='LIFECYCLE_REPLAN_BRAKE';stateEntryTime=t;holdPosition=est.p;
        else
            ...
            if strcmp(resumeState,'TRACK_RTL')
                ...
                if scanNoProgressCount>=cfg.mapMaxNoProgressScans|| ...
                    ...
                    goalUnreachableReason='ROUTE_AFFECTED_NO_ROUTE_BUDGET';
    ...
    if pendingReplan&&pendingExplorationReplan&&~failsafe&& ...
        ...
        elseif suspendedRequestAvailable&&~isempty(fieldnames(suspendedExplorationRequest))
            ...
            % viewpoint on the CURRENT fail-closed grid with the inherited
            % D*/A* + strict trajectory stack, then require fresh admission and
            % execution revalidation. Success creates a NEW authority generation.
            recovery=try_reauthorize_suspended_request_local( ...
                cfg,s24cfg,grid,suspendedExplorationRequest,est,estAcc,t);
            dstarRepairExpanded=dstarRepairExpanded+recovery.stats.dstarExpanded;
            astarScratchExpanded=astarScratchExpanded+recovery.stats.astarExpanded;
            astarRecoveryCount=astarRecoveryCount+recovery.stats.astarRecovery;
            if recovery.success
                planner=recovery.planner;path=recovery.path;traj=recovery.traj;trajClock=0;
                currentTarget=recovery.request.positionXY;segmentIsFinal=false;
                activeExplorationRequest=recovery.request;explorationActive=true;
                activeExplorationAcceptedTime=t;authorityGenerationCounter=authorityGenerationCounter+uint64(1);
                activeAuthorityGeneration=authorityGenerationCounter;
                yawTargetCommand=recovery.request.yaw;lastPlannerMapVersion=double(grid.mapVersion);
                state='TRACK_OUTBOUND';stateEntryTime=t;arrivalConfirmTimer=0;
                executionSuspendedRequestRecoveryCount=executionSuspendedRequestRecoveryCount+1;
                postFaultFreshPlanCount=postFaultFreshPlanCount+1;freshPlanRequiredAfterRevocation=false;
                executionSafetyHistory{end+1}=struct('time',t,'event','REAUTHORIZED_AFTER_REVOCATION', ...
                    'reason',recovery.reason,'requestId',active_request_id(activeExplorationRequest), ...
                    'authorityGeneration',activeAuthorityGeneration); %#ok<AGROW>
                pathHistory{end+1}=path;trajectoryHistory{end+1}=traj; %#ok<AGROW>
                [trajectoryGenerationCount,trajectoryFallbackCount,maxTrajectoryTimeScale,maxReferenceContinuity,maxReplanStateJump,maxReferenceSpeed,maxReferenceAccel,maxReferenceJerk]= ...
                    update_traj_metrics(traj,recovery.jump,trajectoryGenerationCount,trajectoryFallbackCount,maxTrajectoryTimeScale,maxReferenceContinuity,maxReplanStateJump,maxReferenceSpeed,maxReferenceAccel,maxReferenceJerk);
            else
                executionSuspendedRequestRecoveryRejectCount=executionSuspendedRequestRecoveryRejectCount+1;
                executionSafetyHistory{end+1}=struct('time',t,'event','REAUTHORIZATION_REJECTED', ...
                    'reason',recovery.reason,'requestId',active_request_id(suspendedExplorationRequest), ...
                    'authorityGeneration',uint64(0)); %#ok<AGROW>
                state='PLAN_OUTBOUND';stateEntryTime=t;holdPosition=est.p;
            end
            suspendedExplorationRequest=struct();suspendedRequestAvailable=false;suspendedRequestReason='NONE';
        else
            state='PLAN_OUTBOUND';stateEntryTime=t;holdPosition=est.p;
        end
        pendingExplorationTerminal=false;
    end
    if pendingReplan&&~pendingExplorationReplan&&~failsafe&&norm(est.v(1:2))<=cfg.replanBrakeSpeed_mps&& ...
            norm(estAcc(1:2))<=cfg.replanBrakeAccel_mps2&& ...
            t-lastReplanAttempt>=cfg.replanRetryPeriod_s
        ...
        elseif ~routeExists
            ...
            if strcmp(resumeAfterFailure,'TRACK_OUTBOUND')&& ...
                    ...
                    mapExtensionCount>=cfg.mapMaxExtensionAttempts)
                goalUnreachableReason='REPLAN_NO_ROUTE_BUDGET';
                goalUnreachable=true;state='GOAL_UNREACHABLE';stateEntryTime=t;holdPosition=est.p;
            else
    ...
    if pendingReplan&&pendingExplorationReplan&& ...
            t-pendingReplanStart>=cfg.replanBrakeTimeout_s
        pendingReplan=false;pendingExplorationReplan=false;pendingFallbackPath=zeros(0,2);
        gridFallbackActive=false;goalUnreachableReason='AUTHORITY_REPLAN_BRAKE_TIMEOUT';goalUnreachable=true;state='GOAL_UNREACHABLE';
        stateEntryTime=t;holdPosition=est.p;pendingExplorationTerminal=false;
        lastExecutionSafetyReason='AUTHORITY_REPLAN_BRAKE_TIMEOUT';
    end
    if pendingReplan&&~pendingExplorationReplan&&t-pendingReplanStart>=cfg.rtlReplanTimeout_s
        ...
        if ~isempty(timeoutPath)
            ...
            if strcmp(resumeAfterFailure,'TRACK_OUTBOUND')&& ...
                    (scanNoProgressCount>=cfg.mapMaxNoProgressScans|| ...
                    mapExtensionCount>=cfg.mapMaxExtensionAttempts)
                goalUnreachableReason='REPLAN_TIMEOUT_NO_ROUTE_BUDGET';
                goalUnreachable=true;state='GOAL_UNREACHABLE';stateEntryTime=t;holdPosition=est.p;
            else
                state='SCAN_HOLD';stateEntryTime=t;holdPosition=est.p;scanStartYaw=yawCommand;
                if strcmp(resumeAfterFailure,'TRACK_RTL'),scanResumeState='PLAN_RTL';else,scanResumeState='PLAN_OUTBOUND';end
    ...
    switch state
        case 'NAV_DEGRADED_HOLD'
            ...
            if recovered
                % S2.5 v1.0.9: NAV_DEGRADED_HOLD cancels pendingReplan on
                % entry. If it interrupted LIFECYCLE_REPLAN_BRAKE, blindly
                % restoring that state would create a dead brake with no
                % pending work. Re-enter the appropriate planner instead and
                % require a fresh current-state/current-map plan.
                if strcmp(navigationResumeState,'LIFECYCLE_REPLAN_BRAKE')
                    if strcmp(pendingResumeState,'TRACK_RTL')
                        state='PLAN_RTL';
                    else
                        state='PLAN_OUTBOUND';
                    end
                    pendingReplan=false;pendingExplorationReplan=false;
                    pendingExplorationTerminal=false;pendingFallbackPath=zeros(0,2);
                    gridFallbackActive=false;lastReplanAttempt=-inf;
                else
                    state=navigationResumeState;
                end
                stateEntryTime=t;arrivalConfirmTimer=0;
        ...
        case 'INITIAL_HOVER'
            ...
            yawCommand=scanYaw;yawTargetCommand=yawCommand;ref3=hover_ref_yaw(scenario.home,cfg.altitudeNominal_m,yawCommand);
        ...
        case 'PLAN_OUTBOUND'
            ...
            % S2.5 v1.0.4 recovery: after all configured stationary scans,
            % try a fail-closed informative relocation on CURRENT known-free
            % space. Candidate selection is exhaustive safety-first: a ranked
            % endpoint is not committed until the actual metric route, route
            % freshness/dynamic checks, strict trajectory and stop reserve pass.
            segmentRecoveryRelocation=false;
            if ~routeExists&&scanNoProgressCount>=cfg.mapMaxNoProgressScans&& ...
                    mapExtensionCount<cfg.mapMaxExtensionAttempts
                informativeRecoveryAttemptCount=informativeRecoveryAttemptCount+1;
                [rp,rpath,rtraj,rst,rjump,rroute,rmeta]= ...
                    plan_recovery_viewpoint_S2_5(cfg,s24cfg,grid,est,estAcc, ...
                    segmentMissionGoal,scenario.trajectoryInitialTimeScale);
                if isfield(rmeta,'recoveryDiagnostics')
                    lastInformativeRecoveryDiagnostics=rmeta.recoveryDiagnostics;
                end
                st.dstarExpanded=st.dstarExpanded+rst.dstarExpanded;
                st.astarExpanded=st.astarExpanded+rst.astarExpanded;
                st.astarRecovery=st.astarRecovery+rst.astarRecovery;
                if rroute
                    planner=rp;path=rpath;traj=rtraj;jump=rjump;routeExists=true;
                    planMeta=rmeta;plannedRequest=struct();
                    informativeRecoveryRelocationCount=informativeRecoveryRelocationCount+1;
                    segmentRecoveryRelocation=true;
                end
            end
            dstarRepairExpanded=dstarRepairExpanded+st.dstarExpanded;
            astarScratchExpanded=astarScratchExpanded+st.astarExpanded;
            ...
            if isfield(planMeta,'decisionEvidence')&& ...
                    ~isempty(planMeta.decisionEvidence)&& ...
                    isfield(planMeta.decisionEvidence,'candidateCount')&& ...
                    planMeta.decisionEvidence.candidateCount>0
                explorationDecisionHistory{end+1}=struct( ...
                    'time',t,'mapVersion',double(mapState.version), ...
                    'evidence',planMeta.decisionEvidence); %#ok<AGROW>
            end
            if planMeta.frontierUsed,mapExtensionPlanCount=mapExtensionPlanCount+1;end
            if planMeta.explorationUsed
                explorationRequestCount=explorationRequestCount+1;
                requestStatus=validate_exploration_request_S2_4( ...
                    s24cfg,build_execution_grid_S2_4(grid),plannedRequest,t);
                explorationRequestHistory{end+1}=struct('time',t, ...
                    'request',plannedRequest,'status',requestStatus); %#ok<AGROW>
                if requestStatus.valid
                    explorationSelectedCount=explorationSelectedCount+1;
                    ...
                    % Execute the selected sensing orientation while moving
                    % to the viewpoint; the subsequent scan starts here.
                    yawTargetCommand=plannedRequest.yaw;
                else
                    explorationRejectedDecisionCount=explorationRejectedDecisionCount+1;
                    explorationActive=false;activeExplorationRequest=struct();
                    routeExists=false;traj=invalid_traj();path=zeros(0,2);
                end
            elseif planMeta.goalCurrentlyReachable
                explorationActive=false;activeExplorationRequest=struct();
                suspendedExplorationRequest=struct();suspendedRequestAvailable=false;suspendedRequestReason='NONE';
            else
                explorationRejectedDecisionCount=explorationRejectedDecisionCount+1;
            ...
            if executionSafetyEnabled&&freshPlanRequiredAfterRevocation&&routeExists
                postFaultFreshPlanCount=postFaultFreshPlanCount+1;
                freshPlanRequiredAfterRevocation=false;
                executionSafetyHistory{end+1}=struct('time',t,'event','FRESH_PLAN_AFTER_REVOCATION', ...
                    'reason',planMeta.reason,'requestId',active_request_id(activeExplorationRequest), ...
                    'authorityGeneration',activeAuthorityGeneration); %#ok<AGROW>
            end
            ...
            elseif routeExists
                ...
                if scanNoProgressCount>cfg.mapMaxNoProgressScans||mapExtensionCount>=cfg.mapMaxExtensionAttempts
                    goalUnreachableReason='PLAN_OUTBOUND_NO_ROUTE_BUDGET';
                    goalUnreachable=true;state='GOAL_UNREACHABLE';stateEntryTime=t;holdPosition=est.p;
                else
                    state='SCAN_HOLD';stateEntryTime=t;scanResumeState='PLAN_OUTBOUND';scanStartYaw=yawCommand;
                    ...
                    if segmentIsFinal
                        ...
                        if explorationActive&&~isempty(fieldnames(activeExplorationRequest))
                            ...
                            executedFrontierTrackIds(end+1)= ...
                                uint64(activeExplorationRequest.frontierTrackId); %#ok<AGROW>
                        end
                        explorationActive=false;activeExplorationRequest=struct();activeExplorationAcceptedTime=nan;
                        activeAuthorityGeneration=uint64(0);suspendedExplorationRequest=struct();
                        suspendedRequestAvailable=false;suspendedRequestReason='NONE';
                        if segmentRecoveryRelocation
                            % A successful recovery relocation establishes a new
                            % sensing context. The previous hold point's exhausted
                            % scan budget must not be carried into this location.
                            scanNoProgressCount=0;
                            segmentRecoveryRelocation=false;
                        elseif extensionProgress>=cfg.mapMinFrontierProgress_m
                            scanNoProgressCount=0;
                        else
                            scanNoProgressCount=scanNoProgressCount+1;
                        end
        ...
        case 'SCAN_HOLD'
            motorsActive=true;
            scanYaw=wrap_pi_S2_2(scanStartYaw+cfg.mapScanYawRate_radps*(t-stateEntryTime));
            yawCommand=scanYaw;yawTargetCommand=yawCommand;ref3=hover_ref_yaw(holdPosition(1:2).',cfg.altitudeNominal_m,yawCommand);
        ...
        case 'MAP_DEGRADED_HOLD'
            ...
            if isfinite(perceptionAge)&&perceptionAge<=cfg.mapPerceptionHoldTimeout_s
                if any(strcmp(perceptionResumeState,{'TRACK_OUTBOUND','TRACK_RTL'}))
                    if strcmp(perceptionResumeState,'TRACK_RTL')
                        state='PLAN_RTL';stateEntryTime=t;
                    elseif executionSafetyEnabled&&suspendedRequestAvailable
                        % The pre-loss exploration authority was revoked. After
                        % perception recovers, route it through the same bounded
                        % brake/current-grid reauthorization seam; never resume
                        % the old generation directly.
                        pendingReplan=true;pendingExplorationReplan=true;
                        pendingResumeState='TRACK_OUTBOUND';pendingReplanStart=t;
                        lastReplanAttempt=t;pendingFallbackPath=zeros(0,2);
                        gridFallbackActive=false;state='LIFECYCLE_REPLAN_BRAKE';stateEntryTime=t;
                    else
                        state='PLAN_OUTBOUND';stateEntryTime=t;
                    end
                else
                    state=perceptionResumeState;stateEntryTime=t;
                end
                perceptionLossStart=nan;
    ...
    % G-discovered execution seam: before the controller can consume a
    % translating OUTBOUND or RTL reference, verify that the CURRENT fail-closed
    % grid still authorizes that exact reference sample. This guards the same
    % command boundary on both mission legs and reuses the inherited brake/replan.
    guardedTrackState=state;
    if executionSafetyEnabled&&any(strcmp(state,{'TRACK_OUTBOUND','TRACK_RTL'}))&& ...
            ~reference_known_free_local(grid,ref3.p(1:2).')
        executionReferenceGuardCount=executionReferenceGuardCount+1;
        if strcmp(guardedTrackState,'TRACK_OUTBOUND')
            executionReferenceGuardOutboundCount=executionReferenceGuardOutboundCount+1;
        else
            executionReferenceGuardRTLCount=executionReferenceGuardRTLCount+1;
        end
        lastExecutionSafetyReason='CURRENT_REFERENCE_NOT_KNOWN_FREE';
        if ~isfinite(firstFaultDetectionTime)&&isfinite(firstFaultInjectionTime)
            firstFaultDetectionTime=t;
        end
        if explorationActive&&~isempty(fieldnames(activeExplorationRequest))
            executionAuthorityInvalidationCount=executionAuthorityInvalidationCount+1;
            authorityEpisodeInvalidationCount=authorityEpisodeInvalidationCount+1;
            ...
            executionSafetyHistory{end+1}=struct('time',t,'event','REFERENCE_GUARD_BRAKE', ...
                'reason',lastExecutionSafetyReason,'requestId',uint64(0), ...
                'authorityGeneration',uint64(0)); %#ok<AGROW>
            pendingExplorationReplan=false;pendingExplorationTerminal=false;
        end
        % This is a safety-triggered recovery, not evidence of failed
        % exploration progress. The retry path remains bounded by the
        % inherited replan timeouts and, for exploration authority, F14.
        scanNoProgressCount=0;
        pendingReplan=true;pendingResumeState=guardedTrackState;pendingReplanStart=t;
        lastReplanAttempt=t;pendingFallbackPath=zeros(0,2);gridFallbackActive=false;
        holdPosition=est.p;replanBrakeCount=replanBrakeCount+1;
        state='LIFECYCLE_REPLAN_BRAKE';stateEntryTime=t;
        ref3=struct('p',[est.p(1:2);cfg.altitudeNominal_m], ...
            'v',zeros(3,1),'a',zeros(3,1),'yaw',yawCommand);
    end
    ...
    maxTruthYawRate_radps=max(maxTruthYawRate_radps,abs(truth.omega(3)));
    pathLength=pathLength+norm(truth.p-oldTruthP);
    [packet,sensorModel]=simulate_sensor_packet_S2_5(cfg,scenario,truth,sensorModel,t,k);
    ...
    if any(strcmp(state,{'TRACK_OUTBOUND','TRACK_RTL'}))
        ...
        unsafeReference=ix<1||ix>grid.nx||iy<1||iy>grid.ny;
        if ~unsafeReference,unsafeReference=~grid.knownFree(iy,ix)||grid.occ(iy,ix);end
        if unsafeReference
            unknownCommitmentCount=unknownCommitmentCount+1;
            if strcmp(state,'TRACK_OUTBOUND')
                unknownCommitmentOutboundCount=unknownCommitmentOutboundCount+1;
            else
                unknownCommitmentRTLCount=unknownCommitmentRTLCount+1;
            end
            if explorationActive&&strcmp(state,'TRACK_OUTBOUND')
                unsafeViewpointExecutionCount=unsafeViewpointExecutionCount+1;
            end
        end
    end
    if executionSafetyEnabled&&strcmp(state,'TRACK_OUTBOUND')
        staleGenerationActive=revokedAuthorityGeneration>0&& ...
            activeAuthorityGeneration==revokedAuthorityGeneration;
        if pendingExplorationReplan||staleGenerationActive
            staleCommandContinuationCount=staleCommandContinuationCount+1;
        end
    end
...
explorationActiveLog=explorationActiveLog(valid);explorationRequestIdLog=explorationRequestIdLog(valid);
explorationFrontierIdLog=explorationFrontierIdLog(valid);
...
if scenario.expectedPreflightReject
    ...
    % A strict polynomial trajectory is preferred, but the validated grid-
    % route fallback is an intentional safety mechanism for geometrically
    % tight passages. Accept it only together with the unchanged executed-
    % motion, collision, geofence and unknown-space gates below.
    trajectoryGate=trajectoryGenerationCount>=1||gridFallbackCount>=1||scenario.expectedEmergencyLanding;
...
% Keep the inherited S2.3 scan-hold gate unchanged and expose a separate
% S2.5 recovery-aware accounting gate. A successful recovery episode can
% deterministically add the configured no-progress scans plus one mandatory
% scan at the newly reached sensing vantage. No threshold is changed.
recoveryScanHoldAllowance=(cfg.mapMaxNoProgressScans+1)*informativeRecoveryRelocationCount;
s25RecoveryScanHoldPass=scanHoldCount>=minScanHolds&& ...
    scanHoldCount<=maxScanHolds+recoveryScanHoldAllowance;
...
s25MappingCompositePass=mapMetrics.pass&&mapExtensionPass&&mapReplanPass&& ...
    mapSafetyReplanPass&&s25RecoveryScanHoldPass&&perceptionHoldPass&& ...
    promotionPassS23&&unreachablePass&&truthIsolationPass;
minExplorationRequests=field_or_local(scenario,'expectedMinExplorationRequests',0);
minExplorationExecutions=field_or_local(scenario,'expectedMinExplorationExecutions',0);
maxUnsafeViewpointExecutions=field_or_local(scenario,'expectedMaxUnsafeViewpointExecutions',0);
decisionStats=aggregate_exploration_decisions(explorationDecisionHistory);
requireTargetRelevant=logical(field_or_local( ...
    scenario,'expectedTargetRelevantSelection',false));
minCompetingFrontiers=field_or_local( ...
    scenario,'expectedMinCompetingFrontiers',0);
minIrrelevantCandidates=field_or_local( ...
    scenario,'expectedMinIrrelevantCandidates',0);
minDistinctIrrelevantFrontiers=field_or_local( ...
    scenario,'expectedMinDistinctIrrelevantFrontiers',0);
maxIrrelevantSelections=field_or_local( ...
    scenario,'expectedMaxIrrelevantSelections',inf);
targetRelevantSelectionPass=~requireTargetRelevant|| ...
    decisionStats.targetRelevantSelectionCount>=1;
competingFrontierPass=decisionStats.maxCompetingFrontierCount>= ...
    minCompetingFrontiers;
irrelevantCandidatePass=decisionStats.maxIrrelevantCandidateCount>= ...
    minIrrelevantCandidates;
distinctIrrelevantFrontierPass= ...
    decisionStats.maxDistinctIrrelevantFrontierCount>= ...
    minDistinctIrrelevantFrontiers;
irrelevantSelectionPass=decisionStats.irrelevantSelectionCount<= ...
    maxIrrelevantSelections;
explorationDecisionPass=targetRelevantSelectionPass&& ...
    competingFrontierPass&&irrelevantCandidatePass&& ...
    distinctIrrelevantFrontierPass&&irrelevantSelectionPass;
explorationPass=explorationRequestCount>=minExplorationRequests&& ...
    explorationExecutedCount>=minExplorationExecutions&& ...
    unsafeViewpointExecutionCount<=maxUnsafeViewpointExecutions&& ...
    uncertaintyState.truthAccessCount==0&&explorationDecisionPass;
executionSafetyPass=staleCommandContinuationCount==0&&collisionCount==0&& ...
    geofenceViolationCount==0&&unknownCommitmentCount==0&& ...
    unsafeViewpointExecutionCount==0&&mapState.truthAccessCount==0&& ...
    uncertaintyState.truthAccessCount==0;
pass=missionLifecyclePass&&trajectoryGate&&controllerGate&&estimatorGate&&continuityPass&&uncertaintyPass&&staticGate&&mappingPass&&explorationPass&&~stateTimeoutTriggered;
if executionSafetyEnabled,pass=pass&&executionSafetyPass;end
...
summary=struct('lifecycleEnabled',true,'goalReached',goalReached,'pass',pass,'failsafeTriggered',failsafe, ...
    ...
    's25IntegrityDuplicateHitRayCount',perceptionIntegrityDuplicateHitRayCount, ...
    's25IntegrityOcclusionRejectedHitRayCount',perceptionIntegrityOcclusionRejectedHitRayCount, ...
    's25IntegrityKeptHitRayCount',perceptionIntegrityKeptHitRayCount, ...
    ...
    'perceptionHoldCount',perceptionHoldCount, ...
    's25PerceptionStaleBrakeEpisodeCount',perceptionStaleBrakeEpisodeCount, ...
    's25PerceptionSafeResponseCount',perceptionHoldCount+perceptionStaleBrakeEpisodeCount, ...
    'goalUnreachable',goalUnreachable, ...
    'goalUnreachableReason',goalUnreachableReason, ...
    'unknownCommitmentCount',unknownCommitmentCount, ...
    'unknownCommitmentOutboundCount',unknownCommitmentOutboundCount, ...
    'unknownCommitmentRTLCount',unknownCommitmentRTLCount, ...
    'mapPromotionCount',double(mapState.promotedCount), ...
    'explorationRequestCount',explorationRequestCount, ...
    'explorationSelectedCount',explorationSelectedCount, ...
    'explorationExecutedCount',explorationExecutedCount, ...
    'explorationRejectedDecisionCount',explorationRejectedDecisionCount, ...
    'unsafeViewpointExecutionCount',unsafeViewpointExecutionCount, ...
    'executionSafetyEnabled',executionSafetyEnabled, ...
    'executionSafetyPass',executionSafetyPass, ...
    'executionRevalidationCount',executionRevalidationCount, ...
    ...
    'informativeRecoveryRelocationCount',informativeRecoveryRelocationCount, ...
    'informativeRecoveryAttemptCount',informativeRecoveryAttemptCount, ...
    'lastInformativeRecoveryDiagnostics',lastInformativeRecoveryDiagnostics, ...
    ...
    'executionPerceptionRevocationCount',executionPerceptionRevocationCount, ...
    'executionReferenceGuardCount',executionReferenceGuardCount, ...
    'executionReferenceGuardOutboundCount',executionReferenceGuardOutboundCount, ...
    'executionReferenceGuardRTLCount',executionReferenceGuardRTLCount, ...
    'authorityBoundExplorationSuppressed',authorityBoundExplorationSuppressed, ...
    'executionSuspendedRequestRecoveryCount',executionSuspendedRequestRecoveryCount, ...
    'executionSuspendedRequestRecoveryRejectCount',executionSuspendedRequestRecoveryRejectCount, ...
    'postFaultValidRevalidationCount',postFaultValidRevalidationCount, ...
    'postFaultFreshPlanCount',postFaultFreshPlanCount, ...
    'authorityGenerationCount',double(authorityGenerationCounter), ...
    'revokedAuthorityGeneration',double(revokedAuthorityGeneration), ...
    'staleCommandContinuationCount',staleCommandContinuationCount, ...
    'faultInjectionCount',faultInjectionCount, ...
    'faultInjectedAuthorityGenerations',double(faultInjectedAuthorityGenerations), ...
    'firstExplorationAcceptedTime_s',firstExplorationAcceptedTime, ...
    'firstFaultInjectionTime_s',firstFaultInjectionTime, ...
    'firstFaultDetectionTime_s',firstFaultDetectionTime, ...
    'lastExecutionSafetyReason',lastExecutionSafetyReason, ...
    'lastExecutionStoppingDistance_m',lastExecutionStoppingDistance_m, ...
    'lastExecutionRemainingRouteLength_m',lastExecutionRemainingRouteLength_m, ...
    'lastExecutionRouteStopReserveSafe',lastExecutionRouteStopReserveSafe, ...
    'lastExecutionTerminalOverrunNeeded_m',lastExecutionTerminalOverrunNeeded_m, ...
    'lastExecutionTerminalOverrunReserveSafe',lastExecutionTerminalOverrunReserveSafe, ...
    'dynamicPredictionLiveSupported',executionSafetyEnabled&& ...
        field_or_local(executionSafetyCfg,'dynamicPredictionLiveSupported',false), ...
    'executedFrontierTrackIds',executedFrontierTrackIds, ...
    'requiredMinExplorationRequests',minExplorationRequests, ...
    'requiredMinExplorationExecutions',minExplorationExecutions, ...
    'requiredMaxUnsafeViewpointExecutions',maxUnsafeViewpointExecutions, ...
    'explorationDecisionCount',decisionStats.decisionCount, ...
    'competingDecisionCount',decisionStats.competingDecisionCount, ...
    'targetRelevantSelectionCount',decisionStats.targetRelevantSelectionCount, ...
    'irrelevantSelectionCount',decisionStats.irrelevantSelectionCount, ...
    'maxCompetingFrontierCount',decisionStats.maxCompetingFrontierCount, ...
    'maxIrrelevantCandidateCount',decisionStats.maxIrrelevantCandidateCount, ...
    'maxDistinctIrrelevantFrontierCount',decisionStats.maxDistinctIrrelevantFrontierCount, ...
    'maxTargetRelevantCandidateCount',decisionStats.maxTargetRelevantCandidateCount, ...
    'firstSelectedTargetRelevance',decisionStats.firstSelectedTargetRelevance, ...
    'firstSelectedInformationGain',decisionStats.firstSelectedInformationGain, ...
    'firstSelectedTier',decisionStats.firstSelectedTier, ...
    'firstBestIrrelevantUtility',decisionStats.firstBestIrrelevantUtility, ...
    'firstBestIrrelevantInformationGain',decisionStats.firstBestIrrelevantInformationGain, ...
    'firstRawUtilityMarginVsBestIrrelevant',decisionStats.firstRawUtilityMarginVsBestIrrelevant, ...
    'firstPriorityTierMarginVsBestIrrelevant',decisionStats.firstPriorityTierMarginVsBestIrrelevant, ...
    'requiredTargetRelevantSelection',requireTargetRelevant, ...
    'requiredMinCompetingFrontiers',minCompetingFrontiers, ...
    'requiredMinIrrelevantCandidates',minIrrelevantCandidates, ...
    'requiredMinDistinctIrrelevantFrontiers',minDistinctIrrelevantFrontiers, ...
    'requiredMaxIrrelevantSelections',maxIrrelevantSelections, ...
    'targetRelevantSelectionPass',targetRelevantSelectionPass, ...
    'competingFrontierPass',competingFrontierPass, ...
    'irrelevantCandidatePass',irrelevantCandidatePass, ...
    'distinctIrrelevantFrontierPass',distinctIrrelevantFrontierPass, ...
    'irrelevantSelectionPass',irrelevantSelectionPass, ...
    'explorationDecisionPass',explorationDecisionPass, ...
    'explorationPass',explorationPass, ...
    ...
    's25RecoveryScanHoldPass',s25RecoveryScanHoldPass,'recoveryScanHoldAllowance',recoveryScanHoldAllowance, ...
    ...
    'maxTruthYawRate_degps',rad2deg(maxTruthYawRate_radps), ...
    ...
    'trajectoryGate',trajectoryGate,'controllerGate',controllerGate,'estimatorGate',estimatorGate, ...
    'staticGate',staticGate,'mappingCompositePass',mappingPass, ...
    's25MappingCompositePass',s25MappingCompositePass, ...
    ...
    's25SensorFaultApplicationCount',field_or_local(sensorModel,'s25FaultApplicationCount',0), ...
    's25SensorFirstFaultTime_s',field_or_local(sensorModel,'s25FirstFaultTime_s',nan), ...
    's25SensorLastFaultTime_s',field_or_local(sensorModel,'s25LastFaultTime_s',nan), ...
    's25PerceptionFaultApplicationCount',field_or_local(perceptionModel,'s25FaultApplicationCount',0), ...
    's25PerceptionFirstFaultTime_s',field_or_local(perceptionModel,'s25FirstFaultTime_s',nan), ...
    's25PerceptionLastFaultTime_s',field_or_local(perceptionModel,'s25LastFaultTime_s',nan), ...
    's25LaneRejectedCounts',[nav.lanes.rejectedCount], ...
    's25LaneAcceptedCounts',[nav.lanes.acceptedCount]);

log=struct('t',T,'truthP',Ptrue,'truthV',Vtrue,'truthA',Atrue,'truthJ',Jtrue,'truthQ',Qtrue,'truthRpy',RpyTrue, ...
    'estP',Pest,'estV',Vest,'estQ',Qest,'estRpy',RpyEst,'pRef',Pref,'vRef',Vref,'aRef',Aref,'yawRef',yawRefLog, ...
    ...
    'explorationActive',explorationActiveLog,'explorationRequestId',explorationRequestIdLog, ...
    'explorationFrontierTrackId',explorationFrontierIdLog, ...
...
maps=struct('finalGrid',grid,'activeObstacles',activeObstacles,'planner',planner, ...
    ...
    'uncertaintySidecar',uncertaintyState,'frontierState',frontierState, ...
    'explorationRequestHistory',{explorationRequestHistory}, ...
    'explorationDecisionHistory',{explorationDecisionHistory}, ...
    'executionSafetyHistory',{executionSafetyHistory});
    ...
    function store_sample()
        ...
        Pref(k,:)=ref3.p.';Vref(k,:)=ref3.v.';Aref(k,:)=ref3.a.';yawRefLog(k)=ref3.yaw;trackingLog(k)=norm(truth.p-ref3.p);estErrorLog(k)=norm(est.p-truth.p);
        ...
        explorationActiveLog(k)=explorationActive;
        if explorationActive&&~isempty(fieldnames(activeExplorationRequest))
            explorationRequestIdLog(k)=uint64(activeExplorationRequest.requestId);
            explorationFrontierIdLog(k)=uint64(activeExplorationRequest.frontierTrackId);
        end
...
function yawNext=slew_yaw_reference_local(yawNow,yawTarget,maxRate_radps,dt)
% SLEW_YAW_REFERENCE_LOCAL Shortest-path bounded yaw reference transition.
yawNow=wrap_pi_S2_2(double(yawNow));yawTarget=wrap_pi_S2_2(double(yawTarget));
maxStep=max(0,double(maxRate_radps))*max(0,double(dt));
err=wrap_pi_S2_2(yawTarget-yawNow);
if abs(err)<=maxStep||maxStep<=0
    if maxStep<=0,yawNext=yawNow;else,yawNext=yawTarget;end
else
    yawNext=wrap_pi_S2_2(yawNow+sign(err)*maxStep);
end
end

function tf=reference_known_free_local(grid,xy)
% Exact current reference cell authorization on the fail-closed grid.
tf=false;xy=double(xy(:).');
if numel(xy)~=2||any(~isfinite(xy)),return,end
ix=round((xy(1)-grid.xs(1))/grid.resolution)+1;
iy=round((xy(2)-grid.ys(1))/grid.resolution)+1;
if ix<1||ix>grid.nx||iy<1||iy>grid.ny,return,end
tf=logical(grid.knownFree(iy,ix))&&~logical(grid.occ(iy,ix));
end


function id=active_request_id(request)
id=uint64(0);
if isstruct(request)&&~isempty(fieldnames(request))&&isfield(request,'requestId')
    id=uint64(request.requestId);
end
end

function f=route_progress_fraction(request,currentXY)
% Arc-length progress on the accepted metric route, clamped to [0,1].
f=0;
if ~isstruct(request)||~isfield(request,'knownFreeRouteXY'),return,end
p=double(request.knownFreeRouteXY);currentXY=double(currentXY);
if isempty(p)||size(p,2)~=2||size(p,1)<2||any(~isfinite(p(:)))||any(~isfinite(currentXY)),return,end
seg=diff(p,1,1);lens=vecnorm(seg,2,2);total=sum(lens);if total<=eps,return,end
best=inf;along=0;prefix=0;
for i=1:size(seg,1)
    d=seg(i,:);den=dot(d,d);
    if den<=eps,u=0;q=p(i,:);else,u=max(0,min(1,dot(currentXY-p(i,:),d)/den));q=p(i,:)+u*d;end
    dist=norm(currentXY-q);
    if dist<best,best=dist;along=prefix+u*lens(i);end
    prefix=prefix+lens(i);
end
f=max(0,min(1,along/total));
end

function stats=aggregate_exploration_decisions(history)
stats=struct( ...
    'decisionCount',0, ...
    'competingDecisionCount',0, ...
    'targetRelevantSelectionCount',0, ...
    'irrelevantSelectionCount',0, ...
    'maxCompetingFrontierCount',0, ...
    'maxIrrelevantCandidateCount',0, ...
    'maxDistinctIrrelevantFrontierCount',0, ...
    'maxTargetRelevantCandidateCount',0, ...
    'firstSelectedTargetRelevance',0, ...
    'firstSelectedInformationGain',0, ...
    'firstSelectedTier',inf, ...
    'firstBestIrrelevantUtility',-inf, ...
    'firstBestIrrelevantInformationGain',0, ...
    'firstRawUtilityMarginVsBestIrrelevant',nan, ...
    'firstPriorityTierMarginVsBestIrrelevant',nan);
firstSelectedRecorded=false;
for i=1:numel(history)
    if ~isfield(history{i},'evidence')||isempty(history{i}.evidence)
        continue
    end
    e=history{i}.evidence;
    stats.decisionCount=stats.decisionCount+1;
    stats.competingDecisionCount=stats.competingDecisionCount+double(e.hasCompetition);
    stats.targetRelevantSelectionCount=stats.targetRelevantSelectionCount+ ...
        double(e.selectedTargetRelevant);
    stats.irrelevantSelectionCount=stats.irrelevantSelectionCount+ ...
        double(e.irrelevantSelected&&e.selected);
    stats.maxCompetingFrontierCount=max( ...
        stats.maxCompetingFrontierCount,double(e.frontierCount));
    stats.maxIrrelevantCandidateCount=max( ...
        stats.maxIrrelevantCandidateCount,double(e.irrelevantCandidateCount));
    stats.maxDistinctIrrelevantFrontierCount=max( ...
        stats.maxDistinctIrrelevantFrontierCount, ...
        double(e.distinctIrrelevantFrontierCount));
    stats.maxTargetRelevantCandidateCount=max( ...
        stats.maxTargetRelevantCandidateCount, ...
        double(e.targetRelevantCandidateCount));
    if e.selected&&~firstSelectedRecorded
        firstSelectedRecorded=true;
        stats.firstSelectedTargetRelevance=double(e.selectedTargetRelevance);
        stats.firstSelectedInformationGain=double(e.selectedInformationGain);
        stats.firstSelectedTier=double(e.selectedTier);
        stats.firstBestIrrelevantUtility=double(e.bestIrrelevantUtility);
        stats.firstBestIrrelevantInformationGain= ...
            double(e.bestIrrelevantInformationGain);
        stats.firstRawUtilityMarginVsBestIrrelevant= ...
            double(e.rawUtilityMarginVsBestIrrelevant);
        stats.firstPriorityTierMarginVsBestIrrelevant= ...
            double(e.priorityTierMarginVsBestIrrelevant);
    end
end
end
...
function v=field_or_local(s,name,defaultValue)
if isfield(s,name),v=double(s.(name));else,v=double(defaultValue);end
end
...
function rec=try_reauthorize_suspended_request_local(cfg,c,grid,proposal,est,estAcc,tNow)
% A revoked request is proposal data only. Re-issue authority only after a
% CURRENT inherited plan, admission check and execution-time revalidation.
rec=struct('success',false,'reason','NO_SUSPENDED_REQUEST','planner',[], ...
    'path',zeros(0,2),'traj',invalid_traj(),'jump',inf(1,3), ...
    'stats',struct('dstarExpanded',0,'astarExpanded',0,'astarRecovery',0), ...
    'request',struct());
if ~isstruct(proposal)||isempty(fieldnames(proposal))|| ...
        ~isfield(proposal,'positionXY')||numel(proposal.positionXY)~=2|| ...
        any(~isfinite(double(proposal.positionXY)))
    return
end
[pl,p,tr,st,jump,routeExists]=plan_segment( ...
    cfg,grid,est,estAcc,double(proposal.positionXY),1.0);
rec.planner=pl;rec.path=p;rec.traj=tr;rec.jump=jump;rec.stats=st;
if ~routeExists||isempty(p)||~tr.valid
    rec.reason='CURRENT_VIEWPOINT_ROUTE_UNAVAILABLE';return
end
req=proposal;
req.valid=true;req.action='MOVE_TO_VIEWPOINT';req.statusReason='REAUTHORIZED_CURRENT_GRID';
req.knownFreeRouteXY=double(p);req.pathCells=metric_path_cells_local(grid,p);
if isempty(req.pathCells)
    rec.reason='REAUTHORIZED_ROUTE_CELL_CONVERSION_FAILED';return
end
% Keep the original safe retreat anchor, but recompute the route from the
% current stopped position using the inherited A* implementation.
if ~isfield(req,'startXY')||numel(req.startXY)~=2||any(~isfinite(double(req.startXY)))
    rec.reason='REAUTHORIZED_RETREAT_ANCHOR_INVALID';return
end
anchor=double(req.startXY(:).');
[rawRet,~]=astar_grid_S2_2(grid,est.p(1:2).',anchor);
ret=prepare_path_local(grid,est.p(1:2).',rawRet);
if isempty(ret)
    rec.reason='REAUTHORIZED_RETREAT_UNAVAILABLE';return
end
req.retreatRouteXY=ret;req.mapVersion=uint32(grid.mapVersion);
req.createdTime_s=tNow;req.validUntil_s=tNow+double(c.requestValidity_s);
req.lastRevalidatedTime_s=tNow;
g24=build_execution_grid_S2_4(grid);
admission=validate_exploration_request_S2_4(c,g24,req,tNow);
if ~admission.valid
    rec.reason=['READMISSION_' admission.reason];return
end
[execStatus,req2]=revalidate_active_exploration_request_S2_4_F( ...
    cfg,c,g24,req,est.p(1:2).',est.v(1:2),tNow);
if ~execStatus.valid
    rec.reason=['RUNTIME_REVALIDATION_' execStatus.reason];return
end
rec.success=true;rec.reason='CURRENT_GRID_NEW_AUTHORITY';rec.request=req2;
end

function cells=metric_path_cells_local(grid,path)
cells=zeros(0,2);path=double(path);
if isempty(path)||size(path,2)~=2||any(~isfinite(path(:))),return,end
raw=zeros(size(path,1),2);
for i=1:size(path,1)
    ix=round((path(i,1)-grid.xs(1))/grid.resolution)+1;
    iy=round((path(i,2)-grid.ys(1))/grid.resolution)+1;
    if ix<1||ix>grid.nx||iy<1||iy>grid.ny,cells=zeros(0,2);return,end
    raw(i,:)=[iy ix];
end
keep=true(size(raw,1),1);
for i=2:size(raw,1),keep(i)=any(raw(i,:)~=raw(i-1,:));end
cells=raw(keep,:);
if size(cells,1)<2,cells=zeros(0,2);end
end
