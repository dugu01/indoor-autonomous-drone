...
def test_grid_fallback_is_validated_trajectory_mechanism():
    src=(ROOT/'mission_lifecycle_manager_S2_3.m').read_text()
    assert 'trajectoryGenerationCount>=1||gridFallbackCount>=1||scenario.expectedEmergencyLanding' in src

def test_dead_end_replan_is_conditional():
    src=(ROOT/'scenario_S2_3.m').read_text()
    block=src.split("scenario.name='DEAD_END_RECOVERY'",1)[1].split("case {'goal_requires_scan'",1)[0]
    assert 'scenario.expectedMapReplan=false;' in block

def test_dynamic_promotion_replan_is_conditional():
    src=(ROOT/'scenario_S2_3.m').read_text()
    block=src.split("scenario.name='DYNAMIC_TO_STATIC_MAPPING'",1)[1].split("case {'rtl_obstacle_online'",1)[0]
    assert 'scenario.expectedDynamicPromotion=true;' in block
    assert 'scenario.expectedMapReplan=false;' in block
