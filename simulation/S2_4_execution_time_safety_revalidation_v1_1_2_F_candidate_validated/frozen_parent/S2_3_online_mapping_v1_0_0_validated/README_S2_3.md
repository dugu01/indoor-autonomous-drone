...
## Release-closing validation
Run `python3 python_tests/release_end_to_end_backtest.py` before MATLAB. The package is eligible for MATLAB deterministic validation only when it reports `12/12 PASS`.

## One-command complete release gate
Run `release = validate_S2_3_release_all(true);`. It runs the Python 12-case contract backtest, 12 deterministic cases, exact mapper replay, 60 critical S2.3 trials, frozen S2.2 regression suites, and aggregate hard-safety checks. A release report is saved under the S2.3 validation results folder.
