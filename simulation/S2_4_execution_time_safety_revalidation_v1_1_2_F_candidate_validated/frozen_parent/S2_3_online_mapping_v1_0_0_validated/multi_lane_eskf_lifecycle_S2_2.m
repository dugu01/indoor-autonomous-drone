...
n=numel(lane.nisNormalized);first=max(1,n-cfg.imuAttributionWindow+1);
recent=lane.nisNormalized(first:n);recent=recent(isfinite(recent));
if isempty(recent),recentScore=base;else,recentScore=max(min(recent,8));end
w=min(1,max(0,cfg.imuAttributionRecentWeight));score=(1-w)*base+w*recentScore;
end
...
out=struct('p',p,'v',v,'q',q,'omega',omega,'activeLane',nav.selector.activeLane, ...
    ...
    'degraded',false,'rtlRequested',nav.rtlRequested,'imuSuspect',nav.imuSuspect,'imuFaultDetectedTime',nav.imuFaultDetectedTime, ...
    'faultAwareBlend',nav.selector.blendFaultAware,'blendDuration_s',nav.selector.blendDuration_s, ...
