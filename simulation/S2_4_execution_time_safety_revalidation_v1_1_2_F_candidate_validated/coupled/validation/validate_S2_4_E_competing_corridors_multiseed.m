function report = validate_S2_4_E_competing_corridors_multiseed(seeds,benchmarkSeed)
% VALIDATE_S2_4_E_COMPETING_CORRIDORS_MULTISEED Layered physical robustness gate.
% S2_4_E_LAYERED_V6
%
% The fixed benchmark seed must physically instantiate a clean Tier-3 decoy
% and select the Tier-1 target. Other seeds are robustness trials: mission,
% target selection, no irrelevant selection, hard safety, and truth isolation
% must all pass. Stochastic non-appearance of the clean decoy in a robustness
% seed is reported, not treated as a mission failure.
if nargin<1||isempty(seeds),seeds=0:9;end
if nargin<2||isempty(benchmarkSeed),benchmarkSeed=0;end
...
seeds=seeds(:);n=numel(seeds);assert(any(seeds==benchmarkSeed),'S2_4:BenchmarkSeedMissing','Benchmark seed %d must be included.',benchmarkSeed);
runs=cell(n,1);Seed=seeds;
...
for k=1:n
    ...
    [e,found,q]=findEvidence(runs{k}.maps.explorationDecisionHistory);
    ...
    CleanDecoyFound(k)=double(found);
    if ~isempty(e)
    ...
    if found
        ...
        if ~isempty(e)&&e.selectedInformationGain>0,PhysicalDecoyMoreInformativeObserved(k)=double(q.informationGain>e.selectedInformationGain);end
    end
...
benchmarkIdx=find(seeds==benchmarkSeed,1,'first');
benchmarkActivationPass=CleanDecoyFound(benchmarkIdx)==1&&CompetingDecisions(benchmarkIdx)>=1&& ...
    MaxFrontiers(benchmarkIdx)>=2&&DistinctIrrelevantFrontiers(benchmarkIdx)>=1&& ...
    TargetSelections(benchmarkIdx)>=1&&IrrelevantSelections(benchmarkIdx)==0&& ...
    SelectedTier(benchmarkIdx)==1&&SelectedTargetRelevance(benchmarkIdx)>0&& ...
    isfinite(TierPriorityMargin(benchmarkIdx))&&TierPriorityMargin(benchmarkIdx)>0;

% Robustness requirement: every seed must complete the target-directed mission
% without selecting an irrelevant frontier. Clean-decoy activation itself is
% stochastic and is therefore conditioned on, not forced in every seed.
allSeedDecisionPass=all(TargetSelections>=1)&&all(IrrelevantSelections==0)&&all(SelectedTier==1)&&all(SelectedTargetRelevance>0);
activated=CleanDecoyFound==1;
conditionalPriorityPass=all(~activated | (CompetingDecisions>=1 & MaxFrontiers>=2 & ...
    DistinctIrrelevantFrontiers>=1 & isfinite(TierPriorityMargin) & TierPriorityMargin>0));
...
report=struct();report.schema='S2_4_E_LAYERED_MULTI_SEED_V6';report.scenario='active_competing_corridors';
cfg=init_S2_4_E_config();report.version=cfg.version;report.seeds=seeds;report.benchmarkSeed=benchmarkSeed;report.runs=runs;report.summaryTable=summaryTable;
report.adversarialPolicyContractPass=policy.pass;report.literalGeometryPass=geometry.pass;
report.benchmarkActivationPass=benchmarkActivationPass;report.allSeedDecisionPass=allSeedDecisionPass;
report.conditionalPriorityPass=conditionalPriorityPass;report.hardSafetyPass=hardSafetyPass;report.missionCompletionPass=missionCompletionPass;
report.cleanDecoyActivationCount=sum(CleanDecoyFound);report.cleanDecoyActivationSeeds=seeds(activated);
report.physicalDecoyMoreInformativeObservedCount=sum(PhysicalDecoyMoreInformativeObserved);
report.pass=policy.pass&&geometry.pass&&benchmarkActivationPass&&allSeedDecisionPass&&conditionalPriorityPass&&hardSafetyPass&&missionCompletionPass;
fprintf('\nS2.4-E LAYERED COMPETING CORRIDORS MULTISEED V6: %d/%d MISSION PASS | clean-decoy activation %d/%d | benchmark=%d | policy=%d | %s\n', ...
    sum(MissionPass),n,sum(CleanDecoyFound),n,benchmarkActivationPass,policy.pass,localTernary(report.pass,'PASS','FAIL'));
assert(report.pass,'S2_4:CompetingCorridorsGateFailed','One or more layered benchmark, mission, policy or safety checks failed.');
end
