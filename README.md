...
| S2.4-F | Keep re-checking that an exploration move is still safe *while* flying it | 🟡 in MATLAB qualification |
| S2.4-G | Prove the whole S2.2–S2.4 loop end-to-end under an 80-run fault matrix | 🟡 in MATLAB qualification (last run 71/75, fix pending) |
| S2.5 | Stress the estimator + perception with injected sensor faults; confirm safe recovery or safe abort | 🟠 **in progress** |
| S3 | Reactive avoidance of moving obstacles | ⚪ planned |
| S4 | Precision landing on an ArUco marker | ⚪ planned |

**Right now:** finishing the MATLAB qualification of S2.4-F / S2.4-G and
developing S2.5. **Next:** freeze S2.5, then S3 (dynamic obstacles) and S4
(marker landing), then hardware bring-up on ROS 2 / PX4.
...
- **S2.4 active exploration:** physical seed sweep **10/10 PASS** with the
  ...
  frontiers.
- **S2.4-F / S2.4-G (in qualification):** execution-time re-validation of the
  exploration authority, and an end-to-end fault matrix of 5 no-fault + 75 fault
  runs; the last MATLAB campaign scored **71/75**, with the remaining cases
  traced to a yaw-reference transient that is now bounded by a slew-rate limit
  (the estimator threshold is not relaxed).
- **S2.5 (in progress):** injected VIO / LiDAR / IMU / depth dropouts, outlier
  bursts, stale packets and range spikes; every run so far stays collision-,
  geofence- and unknown-commitment-free with the estimator inside its 0.10 m
  budget. Open item: a few fault cases end in a safe controlled abort instead of
  reaching the goal — the recovery / abort behaviour is being finalised.
