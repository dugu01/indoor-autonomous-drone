...
% ANIMATE_S2_2_FLIGHT 3-D truth/estimate/autonomous mission replay.
...
videoFile=fullfile(resultsDir,sprintf('S2_2_%s_%s_animation.mp4',versionLabel,label));
fig=figure('Name',sprintf('S2.2 %s animation — %s',cfg.version,scenario.name), ...
...
if lifecycle
    names={'PREFLIGHT','ARM','TAKEOFF','INITIAL HOVER','WAIT GOAL','PLAN OUTBOUND','TRACK OUTBOUND', ...
        ...
        'PREFLIGHT REJECT','EMERGENCY HOLD','EMERGENCY LAND','REPLAN BRAKE','FAILSAFE'};
else
