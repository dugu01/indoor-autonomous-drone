...
% MISSION_LIFECYCLE_MANAGER_S2_2 Stage S2.2 v0.5.3.1 autonomous lifecycle.
%
% Flow:
    ...
    switch state
        case 'NAV_DEGRADED_HOLD'
            ...
            ref3=struct('p',[est.p(1:2);holdPosition(3)], ...
                'v',zeros(3,1),'a',[brakeAccel(:);0],'yaw',0, ...
        ...
        case 'PLAN_OUTBOUND'
            ...
            % Planning is performed while holding the current airborne pose.
            % Never leave the default ground reference active for the same
            % integration step in which a plan is accepted or rejected.
            ref3=hover_ref(est.p(1:2).',cfg.altitudeNominal_m);
        ...
        case {'TRACK_OUTBOUND','TRACK_RTL'}
            ...
            if gridFallbackActive
                ...
                ref3=struct('p',[est.p(1:2);cfg.altitudeNominal_m], ...
                    'v',[fallbackVelCmd(:);0],'a',[fallbackAccelCmd(:);0],'yaw',0);
        ...
        case 'PLAN_RTL'
            motorsActive=true;
            % Keep a safe airborne hold reference throughout the planning
            % step. This prevents a one-step command toward ground/home when
            % the planner changes state inside this case.
            ref3=hover_ref(est.p(1:2).',cfg.altitudeNominal_m);
            ...
            if ~landingChoice.valid
                ...
                pathHistory{end+1}=path;trajectoryHistory{end+1}=traj; %#ok<AGROW>
                if routeExists&&traj.valid
                    trajClock=0;gridFallbackActive=false;rtlExecuted=true;rtlTrackStart=t;state='TRACK_RTL';stateEntryTime=t;arrivalConfirmTimer=0;
                    ref3=trajectory_start_ref(traj,cfg.altitudeNominal_m);
                    [trajectoryGenerationCount,trajectoryFallbackCount,maxTrajectoryTimeScale,maxReferenceContinuity,maxReplanStateJump,maxReferenceSpeed,maxReferenceAccel,maxReferenceJerk]= ...
                        update_traj_metrics(traj,jump,trajectoryGenerationCount,trajectoryFallbackCount,maxTrajectoryTimeScale,maxReferenceContinuity,maxReplanStateJump,maxReferenceSpeed,maxReferenceAccel,maxReferenceJerk);
                elseif routeExists
                    % A geometric route exists, but the current moving state
                    % cannot be connected safely to it. Brake to a near-hover
                    % and retry instead of declaring an emergency landing.
        ...
        case 'EMERGENCY_HOLD'
            ...
            if emergencyVerticalOnly
                ...
                % Do not begin vertical descent while the jerk-limited
                % controller is still trying to remove horizontal momentum.
                % Mixing a new vertical profile with the XY braking pulse can
                % consume the shared jerk budget and leave a large residual
                % horizontal velocity. Use the short trusted inertial window
                % to finish braking first, then descend level.
                brakeTimeComplete=~isfinite(xyLossBrakeEndTime)||t>=xyLossBrakeEndTime;
                trustedSpeed=norm(est.v(1:2));
                speedSettled=trustedSpeed<=cfg.xyLossBrakeExitSpeed_mps;
                trustExpired=~isfinite(xyLossVelocityTrustEnd)||t>=xyLossVelocityTrustEnd;
                horizontalBrakeComplete=brakeTimeComplete&&(speedSettled||trustExpired);
...
function [a,dampingEnabled]=xy_loss_horizontal_command(t,tBrakeEnd,tTrustEnd,aBrake)
% Confidence-limited XY-loss arrest command.
% The frozen feedforward pulse removes the last aid-bounded velocity, while
% short-term inertial velocity damping corrects controller lag/model mismatch.
% Both are disabled before the blind vertical descent begins.
a=active_xy_loss_brake(t,tBrakeEnd,aBrake);
dampingEnabled=isfinite(tTrustEnd)&&t<=tTrustEnd;
end
...
function ref=trajectory_start_ref(traj,z)
r=sample_min_snap_state_S2_2(traj,0);
ref=struct('p',[r.p(:);z],'v',[r.v(:);0],'a',[r.a(:);0],'yaw',0);
end
...
switch s
    ...
    case 'NAV_DEGRADED_HOLD',id=20;otherwise,id=0;
end
end
