...
ev=ref.v(:)-est.v(:);
refAccel=ref.a(:);
horizontalBlindMode=isfield(ref,'horizontalControlEnabled')&&~ref.horizontalControlEnabled;
if horizontalBlindMode
...
% component; it never adds outward acceleration. It is bypassed in blind
% horizontal mode because post-loss velocity is not an observable signal.
if ~horizontalBlindMode
    aCmd(1:2)=guard_horizontal_speed(cfg,est.v(1:2),aCmd(1:2));
end
