#!/usr/bin/env python3
"""Cumulative static audit for Stage S2.2 v0.5.2.
...
'LIFECYCLE_REPLAN_BRAKE','FAILSAFE','NAV_DEGRADED_HOLD']
checks=[]
...
add('versioned result path',"cfg.version='v0.5.2';" in cfg and "'S2_2_mission_replanning',versionFolder" in run)
try:
...
add('degraded navigation pauses mission before delayed failsafe',
    "case 'NAV_DEGRADED_HOLD'" in life and
    "est.degraded&&armed&&~est.rtlRequested" in life and
    "navigationResumeState=state" in life)
add('failsafe estimator metric stops claiming post-loss observability',
    'maxEstimatorPositionErrorObservable' in life and
    'estimatorPositionErrorAtFailsafeTrigger' in life and
    'maxEstimatorPositionErrorPostLoss' in life and
    'estimatorFailsafeMetric=max' in life)
add('XY-loss response uses no stale position and explicit velocity damping',
    "'horizontalControlEnabled',false" in life and
    "'horizontalVelocityDampingEnabled',cfg.emergencyVelocityDampingEnabled" in life and
...
add('versioned v0.5.2 output conventions','versionFolder' in run and "cfg.version='v0.5.2';" in cfg)
...
semantic_ok,semantic_tail=run_script('s2_2_v0_5_1_runtime_semantic_regression.py')
emergency_ok,emergency_tail=run_script('s2_2_v0_5_2_emergency_regression.py')
...
add('runtime-semantic planning regression',semantic_ok,semantic_tail)
add('position-loss emergency regression',emergency_ok,emergency_tail)
try:
