function [log,summary,maps] = mission_manager_v0_5_3_core_S2_2(cfg,scenario)
% MISSION_MANAGER_V0_5_3_CORE_S2_2 Multi-seed robust legacy-scenario core.
...
[nav,est]=multi_lane_eskf_robust_S2_2('init',cfg,packet,0);
...
timeToGoal=nan;obstacleNoDataElapsed=0;clearTimer=0;wasAvoiding=false;rejoinCount=0;
rejoinEntryTime=nan;rejoinBestGoalDistance=norm(est.p(1:2).'-scenario.goal);
rejoinLastProgressTime=0;progressRecoveryCount=0;maxRejoinDuration=0;
...
for k=2:maxSteps
    ...
    if mapChanged
        ...
        if ~isempty(candidatePath)&&candidateTraj.valid
            ...
            pendingReplan=false;pendingReplanStart=nan;gridFallbackActive=false;
    ...
    if pendingReplan&&~failsafe&& ...
        ...
        if ~isempty(candidatePath)&&candidateTraj.valid
            ...
            rejoinEntryTime=t;rejoinBestGoalDistance=norm(est.p(1:2).'-scenario.goal);rejoinLastProgressTime=t;
            safetyVelCmd=est.v(1:2);safetyAccelCmd=zeros(2,1);
            [trajectoryGenerationCount,trajectoryFallbackCount,maxTrajectoryTimeScale,maxReferenceContinuity,maxReplanStateJump,maxReferenceSpeed,maxReferenceAccel,maxReferenceJerk]= ...
                update_traj_metrics(traj,jump,trajectoryGenerationCount,trajectoryFallbackCount,maxTrajectoryTimeScale,maxReferenceContinuity,maxReplanStateJump,maxReferenceSpeed,maxReferenceAccel,maxReferenceJerk);
        end
    ...
    if pendingReplan&&t-pendingReplanStart>=cfg.replanBrakeTimeout_s
        [rawFallback,aFallback]=astar_grid_S2_2(grid,est.p(1:2).',scenario.goal);
        astarScratchExpanded=astarScratchExpanded+aFallback.expanded;
        fallbackPath=prepare_path(grid,est.p(1:2).',rawFallback);
        if ~isempty(fallbackPath)
            path=fallbackPath;gridFallbackPath=fallbackPath;
            ...
            state='GRID_FALLBACK';wasAvoiding=false;clearTimer=0;
        else
            state='FAILSAFE';failsafe=true;holdPosition=est.p;
        end
    end
    ...
    elseif ~obstacleCoverageOK
        ...
        if ~strcmp(state,'FAILSAFE')&&~strcmp(state,'COMPLETE')
            if gridFallbackActive
                [targetVel,gridFallbackIndex]=grid_fallback_target_v04( ...
                ...
                desiredVel=fallbackVelCmd;trackingError=norm(est.p(1:2).'-scenario.goal);refXY=[];
                state='GRID_FALLBACK';
            else
            ...
            elseif wasAvoiding||strcmp(state,'REJOIN')
                if gridFallbackActive
                    state='GRID_FALLBACK';wasAvoiding=false;clearTimer=0;rejoinEntryTime=nan;
                    safetyVelCmd=desiredVel;safetyAccelCmd=fallbackAccelCmd;
                else
                    state='REJOIN';
                    [safetyVelCmd,safetyAccelCmd]=shape_velocity_command_S2_2( ...
                    ...
                    if isfinite(trackingError)&& ...
                        ...
                        state='TRACK';wasAvoiding=false;clearTimer=0;rejoinEntryTime=nan;
                        safetyVelCmd=est.v(1:2);safetyAccelCmd=zeros(2,1);
                    end
                end
            else
                if gridFallbackActive,state='GRID_FALLBACK';else,state='TRACK';end
            end
        end
    end

    % Liveness watchdog: dynamic avoidance may leave the vehicle unable to
    % re-acquire an old paused trajectory. If REJOIN makes no measurable
    % goal progress, brake and rebuild the route from the current estimate.
    if strcmp(state,'REJOIN')&&~pendingReplan&&~failsafe
        goalDistance=norm(est.p(1:2).'-scenario.goal);
        if ~isfinite(rejoinEntryTime)
            rejoinEntryTime=t;rejoinBestGoalDistance=goalDistance;rejoinLastProgressTime=t;
        end
        if goalDistance<=rejoinBestGoalDistance-cfg.rejoinProgressMinimum_m
            rejoinBestGoalDistance=goalDistance;rejoinLastProgressTime=t;
        end
        maxRejoinDuration=max(maxRejoinDuration,t-rejoinEntryTime);
        stalled=(t-rejoinLastProgressTime)>=cfg.rejoinProgressTimeout_s;
        overlong=(t-rejoinEntryTime)>=cfg.rejoinHardTimeout_s;
        if stalled||overlong
            if progressRecoveryCount<cfg.maxProgressRecoveries
                pendingReplan=true;pendingReplanStart=t;lastReplanAttempt=t;
                replanBrakeCount=replanBrakeCount+1;progressRecoveryCount=progressRecoveryCount+1;
                state='REPLAN_BRAKE';holdPosition=est.p;wasAvoiding=true;clearTimer=0;
                safetyVelCmd=est.v(1:2);safetyAccelCmd=zeros(2,1);
                rejoinEntryTime=nan;
            else
                [rawFallback,aFallback]=astar_grid_S2_2(grid,est.p(1:2).',scenario.goal);
    ...
    oldEstV=est.v;[nav,est]=multi_lane_eskf_robust_S2_2('step',nav,cfg,packet,t,cfg.dt);
...
summary=struct('goalReached',goalReached,'pass',pass,'failsafeTriggered',failsafe,'rtlRequested',rtlRequested, ...
    ...
    'progressRecoveryCount',progressRecoveryCount,'maxRejoinDuration_s',maxRejoinDuration, ...
    ...
    'finalDistanceToGoal_m',norm(truth.p(1:2).'-scenario.goal),'finalState',state, ...
    'imuFaultDetectedTime_s',nav.imuFaultDetectedTime, ...
    'faultSwitchTime_s',fault_switch_time(nav.switchLog),'faultBlendDuration_s',fault_blend_duration(nav.switchLog));
...
elseif any(strcmp(state,{'DYNAMIC_AVOID','DYNAMIC_HOLD','REJOIN','REPLAN_BRAKE','GRID_FALLBACK'}))
...
function [vTarget,index]=grid_fallback_target_v04(cfg,path,index,p,v)
vTarget=zeros(2,1);
...
for i=1:numel(switchLog)
    if isfield(switchLog(i),'faultAware')&&switchLog(i).faultAware,t=switchLog(i).time;return;end
end
end
function d=fault_blend_duration(switchLog)
d=nan;
for i=1:numel(switchLog)
    if isfield(switchLog(i),'faultAware')&&switchLog(i).faultAware,d=switchLog(i).blendDuration_s;return;end
end
end
...
switch s
    ...
    case 'FAILSAFE',id=5;case 'COMPLETE',id=6;case 'REJOIN',id=7;case 'REPLAN_BRAKE',id=8;case 'GRID_FALLBACK',id=9;otherwise,id=0;
end
end
