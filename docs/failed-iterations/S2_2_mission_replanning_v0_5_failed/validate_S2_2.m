...
% VALIDATE_S2_2 Complete v0.4 regression + v0.5 lifecycle matrix.
...
save(fullfile(validationDir,'validation_report_S2_2_v0_5.mat'),'report');
write_validation_text(report,fullfile(validationDir,'validation_report_S2_2_v0_5.txt'));
failed=find(~[report.pass]);
if isempty(failed)
    fprintf('\n[VALIDATE S2.2 v0.5] All %d scenarios passed.\n',numel(scenarios));
    fprintf('[VALIDATE S2.2 v0.5] Report saved: %s\n',validationDir);
else
    names=strjoin({report(failed).scenario},', ');
    fprintf(2,'\n[VALIDATE S2.2 v0.5] %d/%d scenarios failed: %s\n',numel(failed),numel(scenarios),names);
    fprintf('[VALIDATE S2.2 v0.5] Complete report saved: %s\n',validationDir);
    error('S2_2:ValidationFailed','S2.2 v0.5 failed scenarios: %s',names);
end
end

function write_validation_text(report,filePath)
f=fopen(filePath,'w');if f<0,return;end;c=onCleanup(@()fclose(f)); %#ok<NASGU>
fprintf(f,'Stage S2.2 v0.5 validation report\nGenerated: %s\n\n',datestr(now,31));
for i=1:numel(report)
    ...
    if isfield(s,'lifecycleEnabled')&&s.lifecycleEnabled
        ...
        fprintf(f,['%s | PASS %d | goal %d | fail %d | arm %d | takeoff %d | RTL %d | land %d | disarm %d | ' ...
            ...
            'est %.4f/%.3fdeg | clear %.4f | Zexec %.3f/%.3f/%.3f\n'], ...
            ...
            min(s.minObstacleClearance_m,s.minWallClearance_m), ...
